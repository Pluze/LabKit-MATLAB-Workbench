classdef (Sealed) ResultOutput
    %RESULTOUTPUT Declare one validated result-package output.
    %
    % Usage:
    %   output = labkit.ui.ResultOutput(id, role, relativePath, Name=Value)
    %
    % Description:
    %   ResultOutput records App-owned output identity and media semantics.
    ...
    %
    % Outputs:
    %   output - Immutable labkit.ui.ResultOutput value.
    %
    % Errors:
    %   labkit:ui:contract:UnknownArgument - An option is unknown, duplicated,
    %       or unpaired.
    %   labkit:ui:contract:InvalidValue - Text, path, status, or warnings are
    %       malformed.
    %
    % Example:
    ...
    %
    % See also labkit.ui.Result, labkit.ui.RuntimeContext
    ...
    methods
        function obj = ResultOutput(id, role, relativePath, varargin)
            names = ["MediaType", "Status", "Message", "Warnings"];
            options = parseContractOptions( ...
                "labkit.ui.ResultOutput", names, varargin{:});
...
function value = relativePathValue(value)
    ...
    if startsWith(value, "/") || startsWith(value, "//") || ...
        ...
        error("labkit:ui:contract:InvalidValue", ...
            "ResultOutput relativePath must be package-relative.");
    end
    parts = split(value, "/");
    if any(parts == ["", ".", ".."], "all")
        error("labkit:ui:contract:InvalidValue", ...
            "ResultOutput relativePath must not traverse package boundaries.");
    end
    value = join(parts, "/");
end
...
function value = statusValue(value)
    ...
    if ~any(value == ["success", "failed", "skipped"])
        error("labkit:ui:contract:InvalidValue", ...
            "ResultOutput Status must be success, failed, or skipped.");
    end
end
...
function value = nonemptyText(value, name)
    ...
    if strlength(value) == 0
        error("labkit:ui:contract:InvalidValue", ...
            "ResultOutput %s must be nonempty.", name);
    end
end
...
function value = scalarText(value, name)
    if ~(ischar(value) || (isstring(value) && isscalar(value)))
        error("labkit:ui:contract:InvalidValue", ...
            "ResultOutput %s must be scalar text.", name);
    end
    value = string(value);
end
...
function values = textRow(values, name)
    ...
    elseif ~isstring(values)
        error("labkit:ui:contract:InvalidValue", ...
            "ResultOutput %s must be text.", name);
    end
