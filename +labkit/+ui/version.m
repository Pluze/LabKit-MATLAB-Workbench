...
%       and a short maintainer note.
    ...
    info = labkit.contract.versionInfo("ui", "4.0.0", ">=4.0 <5", ...
        "stable", "UI 4.x app/spec/view/tool/diag contract with declarative app definitions, grouped semantic controls, framework-owned runtime dispatch, debounced parameter callbacks, spinner-backed panners, reusable image preview redraws, compact file panels, toolPanel file entry helpers, filePanel title context, preview-area row/column sizing, visible-window early paint, startup readiness state, lazy preview scroll-interaction setup, debug trace, debug artifact sample/output folders, hidden-test-safe alerts, close guard, crash reports, output folder prompts, and default text fitting.");
end
