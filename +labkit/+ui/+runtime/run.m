...
%   declared hydration actions through private startup readiness machinery.
    ...
    layout = buildRuntimeLayout(def, callbacks, state);
...
function layout = buildRuntimeLayout(def, callbacks, state)
    layoutFcn = def.layout;
    n = narginOf(layoutFcn);
    if n == 0
        layout = layoutFcn();
    elseif n == 1
        layout = layoutFcn(callbacks);
    else
        layout = layoutFcn(callbacks, state);
    end
    layout.props.utilities = def.utilities;
end
...
function applyOneEffect(fig, effect)
    ...
    switch string(effect.type)
        ...
        case "alert"
            ...
            labkit.ui.runtime.showAlert(fig, string(effect.message), titleText);
        case "setBusy"
