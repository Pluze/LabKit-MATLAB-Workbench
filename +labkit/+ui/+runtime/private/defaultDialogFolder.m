% Private UI runtime helper. Returns a safe default folder for dialogs.
function folder = defaultDialogFolder(kind, proposedFolder)
%
% App-facing contract:
%   folder = defaultDialogFolder(kind)
%   folder = defaultDialogFolder(kind, proposedFolder)
%
% Inputs:
...
function kind = normalizeKind(kind)
    if ~(ischar(kind) || (isstring(kind) && isscalar(kind)))
        error('labkit:ui:runtime:InvalidDialogKind', ...
