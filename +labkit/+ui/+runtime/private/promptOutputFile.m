% Private UI runtime helper. Prompts for an output file with safe defaults.
function [filepath, cancelled, file, folder] = promptOutputFile(filterSpec, titleText, defaultPath, varargin)
%
% App-facing contract:
%   filepath = promptOutputFile(filterSpec, title, defaultPath)
%   [filepath, cancelled] = promptOutputFile(...)
%   [...] = promptOutputFile(..., "Chooser", chooserFcn)
%
% Inputs:
...
%       through defaultDialogFolder("output", folder).
...
function opts = parseOptions(varargin)
    ...
    if mod(numel(varargin), 2) ~= 0
        error('labkit:ui:runtime:InvalidPromptOutputFileOptions', ...
...
function pathValue = outputDefaultPath(defaultPath)
    ...
    if isempty(defaultPath) || strlength(strtrim(defaultPath(1))) == 0
        folder = defaultDialogFolder("output");
    ...
    safeFolder = defaultDialogFolder("output", folder);
