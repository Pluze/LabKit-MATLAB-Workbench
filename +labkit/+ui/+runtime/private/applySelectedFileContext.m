% Private UI runtime helper. Expected caller: setControlFileSelection.
...
function applySelectedFileContext(ui, id)
    ...
    refreshPreviewTitles(ui, context);
end
...
function refreshPreviewTitles(ui, context)
    ...
    for k = 1:numel(names)
        ...
        for n = 1:numel(axesList)
            ...
            if isgraphics(ax) && isprop(ax, 'Title')
                title(ax, selectedFileContextTitle(ax.Title.String, context));
            end
        end
    end
end

function titleText = selectedFileContextTitle(titleText, context)
