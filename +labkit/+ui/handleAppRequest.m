...
%       launches this is a disabled in-memory logger; for "__labkit_debug__"
%       launches it is enabled and may mirror messages to opts.logFile or
%       opts.logCallback.
        ...
        case "__labkit_debug__"
            ...
            if numel(args) > 2
                error(errorId(appName, 'InvalidTestRequest'), ...
                    '__labkit_debug__ accepts at most one options struct.');
            elseif numel(args) == 2
                opts = args{2};
            end
            opts.enabled = true;
