classdef (Hidden, Sealed) ResultWriter < handle
    ...
    methods (Access = ?labkit.ui.RuntimeKernel)
        ...
        function written = write(obj, folder, result)
            ...
            written = labkit.ui.DialogResult(string(manifestPath));
        end
    end
end
...
function manifest = resultManifest(application, document, result, outputs)
    manifest = struct( ...
        ...
        "provenance", struct( ...
            "labkitUiVersion", string(labkit.ui.version().current), ...
...
function writeAtomic(filepath, manifest)
    try
        ...
        failure = MException("labkit:ui:runtime:InvalidResultManifest", ...
