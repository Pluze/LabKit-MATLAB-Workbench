function runtime = runtime(ax, opts)
%RUNTIME Create a runtime that owns image-axes interaction sessions.
%
% Usage:
...
%   installDefaultCallbacks(), setTraceCallback(fcn), createSession(options),
...
% do not use this runtime.
    ...
    function installDefaultCallbacks()
        if isValidHandle(state.ax)
            labkit.ui.interaction.enablePopout(state.ax);
        end
    ...
    function session = createSession(options)
        ...
        sessionState.name = string(optionValue(options, 'name', 'interaction'));
        sessionState.onPointerDown = optionValue(options, 'onPointerDown', []);
        sessionState.onScroll = optionValue(options, 'onScroll', []);
        sessionState.installScrollWheel = optionValue(options, 'installScrollWheel', true);
        sessionState.scrollTargets = normalizeHandles(optionValue(options, ...
            'scrollTargets', []));
        sessionState.scrollScope = scrollScopeValue(optionValue(options, ...
            'scrollScope', 'targets'));
        ...
        if isfield(options, 'background')
            setSessionBackground(options.background);
        end
        if isfield(options, 'graphics')
            setSessionGraphics(options.graphics);
        end
