...
%
% Usage:
%   runtime = labkit.ui.interaction.runtime(imageAxes);
%   tool = labkit.ui.interaction.scaleBar(parentGrid, 3, runtime, opts);
...
%          scaleBarGeometry(), placedScaleBar(), renderOverlay(), clearScaleBar(),
...
% the app has not already supplied one with setBackground().
    ...
    assert(isstruct(runtime) && isfield(runtime, 'axes') && ...
        ...
        'Third input must be a labkit.ui.interaction.runtime result.');
    ...
    tool.scaleBarGeometry = @scaleBarGeometry;
    ...
    function cal = calibration()
        cal = labkit.ui.interaction.scaleBarCalibration(scalePanel.referencePixels(), ...
    ...
    function geometry = scaleBarGeometry()
        geometry = scalePanel.scaleBarGeometry(state.imageSize);
    end

    function geometry = placedScaleBar()
        geometry = state.scaleBar;
    end
    ...
    function onPanelScaleBarChanged(src, evt)
        ...
        if ~isempty(state.scaleBar)
            try
                state.scaleBar = scaleBarGeometry();
            catch
                state.scaleBar = [];
            end
        end
    ...
    function ensureReferenceEditor()
        ...
        if isempty(state.referenceEditor)
            ...
            state.referenceEditor = labkit.ui.interaction.anchorEditor(state.runtime, state.imageSize, ...
                struct('closed', false, ...
