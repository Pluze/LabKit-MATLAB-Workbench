...
% labkit.ui.interaction.scaleBar. Inputs are parent grid, target row, and options
...
%
% Usage:
%   ui = scaleBarPanel(parentGrid, 2, opts);
%   ui.setReferencePixels(125);
%   cal = ui.calibration();
%   [pxPerUnit, unitName] = ui.pixelsPerUnit();
%   geometry = ui.scaleBarGeometry(size(imageData));
%
% Inputs:
...
%        calibration(), pixelsPerUnit(), scaleBarGeometry(imageSize),
%        updateReadout(), setEnabled(state).
%
%   scaleBarGeometry(imageSize) returns a struct with fields line, label, color,
%   labelPosition, verticalAlignment, pixelsPerUnit, unit, barLength,
%   position, and colorName. Apps own drawing the returned geometry onto axes.
%
...
%   blockPlacement logical fields; omitted fields use interactive defaults.
    ...
    [lblColor, ddColor] = createLabeledDropdown(grid, ...
    ...
    [txtReferencePx, lblReferencePxReadout] = createReadOnlyInfoRow( ...
        grid, 9, 'Reference px:');
    [txtPxPerUnit, lblPxPerUnit] = createReadOnlyInfoRow( ...
        grid, 10, 'Pixels/unit:');
    ...
    ui.scaleBarGeometry = @scaleBarGeometry;
    ...
    function cal = calibration()
        cal = labkit.ui.interaction.scaleBarCalibration(referencePixels(), referenceLength(), ...
            ddUnit.Value, struct('units', {units}, 'defaultUnit', defaultUnit));
    end

    function geometry = scaleBarGeometry(imageSize)
        cal = calibration();
        pxPerUnit = cal.pixelsPerUnit;
        unitName = cal.unit;
