...
% labkit.ui.interaction.anchorEditor. Inputs are existing and candidate image-pixel
...
function [segmentIdx, bestDistance] = nearestVisibleSegment(points, point, imageSize, curveStyle, closed)
    ...
    [curve, owners] = anchorCurvePoints(points, imageSize, curveStyle, closed);
    if size(curve, 1) < 2
        return;
    end
...
function tf = endpointExtensionIntersectsVisiblePath(points, newPoint, imageSize, curveStyle, prepend)
    ...
    [curve, owners] = anchorCurvePoints(points, imageSize, curveStyle, false);
    if size(curve, 1) < 2
        return;
    end
