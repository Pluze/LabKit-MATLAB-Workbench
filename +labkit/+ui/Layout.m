classdef (Sealed) Layout
    ...
    %       Bound controls need no Changed Command for ordinary state updates.
    %       Expressions, indexing, and function calls are not supported.
    %       Default: empty.
    %   SelectionBind - Optional strict state field path for filePanel
    ...
    methods (Static)
        ...
        function obj = field(id, varargin)
            names = ["Kind", "Value", "Choices", "Limits", "Step", "Bind", ...
        ...
        function obj = rangeField(id, varargin)
            options = parseContractOptions("labkit.ui.Layout.rangeField", ...
                ["Value", "Limits", "Enabled", "Bind", "Changed"], varargin{:});
            signal = optionValue(options, "Changed", []);
        ...
        function obj = panner(id, varargin)
            names = ["Value", "Limits", "Step", "ShowTicks", "Bind", ...
                "Enabled", "Changed"];
            options = parseContractOptions( ...
