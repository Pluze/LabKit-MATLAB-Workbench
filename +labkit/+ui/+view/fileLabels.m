...
%
% App-facing contract:
%   labels = labkit.ui.view.fileLabels(paths)
...
function opts = parseOptions(args)
    if mod(numel(args), 2) ~= 0
        error('labkit:ui:view:InvalidFileLabelOptions', ...
...
function paths = normalizePaths(paths)
    ...
    elseif iscell(paths)
        ...
        error('labkit:ui:view:InvalidFileLabelPaths', ...
...
function status = normalizeStatus(status, count)
    ...
    if numel(status) ~= count
        error('labkit:ui:view:InvalidFileLabelStatus', ...
