% Private App plot helper for copied popout-figure controls.
...
for k = 1:numel(handles)
    ...
    try
        handle.FontSize = min(36, max(6, ...
            double(handle.FontSize) + delta));
    catch
    end
end
end
...
for k = 1:numel(handles)
    ...
    try
        handle.LineWidth = max( ...
            0.25, double(handle.LineWidth) + delta);
    catch
    end
end
end
...
try
    ax.LineWidth = max( ...
        0.25, min(5, double(ax.LineWidth) + delta));
catch
end
end
...
try
    ...
    ax.GridAlpha = max( ...
        0.05, min(1, double(ax.GridAlpha) + delta));
    if isprop(ax, 'MinorGridAlpha')
        ax.MinorGridAlpha = max( ...
            0.05, min(1, double(ax.MinorGridAlpha) + delta));
    end
catch
end
end
