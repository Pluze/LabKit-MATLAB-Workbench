classdef (Hidden, Sealed) RuntimeKernel < handle
    ...
    methods
        ...
        function destination = exportDiagnosticBundle( ...
                obj, destination, includeSensitiveDetails, includePrivateState)
            if nargin < 3
            ...
            if includePrivateState && ~includeSensitiveDetails
                error("labkit:app:contract:InvalidValue", ...
                    "Private App state requires complete sensitive logs.");
            end
        ...
        function destination = exportDiagnosticBundleInteractive(obj)
            selection = obj.Context.chooseOption( ...
                "Choose diagnostic detail. Complete logs may contain " + ...
                 "paths, filenames, scientific values, and exception " + ...
                 "locations. Add state MAT only when exact project/session " + ...
                 "values or decoded caches are needed.", ...
                ["Redacted log", ...
            ...
            includeSensitiveDetails = selection.Value ~= "Redacted log";
            includePrivateState = selection.Value == ...
                "Complete log + state MAT (sensitive)";
            destination = "";
            try
                automaticDestination = ...
                    obj.automaticDiagnosticDestination( ...
                        includeSensitiveDetails, includePrivateState);
        ...
        function destination = exportDiagnosticTextFallback( ...
            ...
            includeSensitiveDetails = logicalScalar( ...
                includeSensitiveDetails, "includeSensitiveDetails");
            requestedPrivateState = logicalScalar( ...
                requestedPrivateState, "requestedPrivateState");
            if requestedPrivateState && ~includeSensitiveDetails
                error("labkit:app:contract:InvalidValue", ...
                    "Private App state requires complete sensitive logs.");
            end
            obj.Recorder.log( ...
    ...
    methods (Access = private)
        ...
        function destination = automaticDiagnosticDestination( ...
                obj, includeSensitiveDetails, includePrivateState)
            stem = diagnosticArtifactStem( ...
                includeSensitiveDetails, includePrivateState);
        ...
        function filename = automaticDiagnosticFilename( ...
            ...
            filename = obj.automaticArtifactFilename( ...
                diagnosticArtifactStem( ...
                    includeSensitiveDetails, includePrivateState), ".zip");
        end
...
elseif includeSensitiveDetails
