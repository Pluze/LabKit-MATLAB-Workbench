...
function checkFixedPixelCropPreservesClassAndSize()
    ...
    result = batchImageCropWorkflow("cropImage", img, struct( ...
        'cropWidth', 4, ...
        'cropHeight', 3, ...
        'centerXY', [5, 6], ...
        'angleDeg', 0, ...
...
function checkSelectedFileNormalization()
    ...
    paths = batchImageCropWorkflow( ...
        "selectedBatchCropImagePaths", {'frame_b.png', 'frame_a.tif'}, folder);
    names = fileNames(paths);
    assert(isequal(names, {'frame_a.tif'; 'frame_b.png'}), ...
        'Selected batch crop images should be sorted by filename.');

    assertThrows(@() batchImageCropWorkflow( ...
        "selectedBatchCropImagePaths", 'notes.txt', folder), ...
...
function checkManifestContract()
    result = batchImageCropWorkflow("cropImage", uint8(ones(5, 6)), struct( ...
        'cropWidth', 3, ...
        'cropHeight', 4, ...
        'centerXY', [3, 3], ...
        'angleDeg', 0));
    ...
    T = batchImageCropWorkflow("buildBatchCropManifest", result);
...
function checkExportWritesUniqueOutputs()
    ...
    payload = batchImageCropWorkflow("writeBatchCropOutputs", item, struct( ...
