classdef LegacyImageCurvatureMeasurementTest < matlab.unittest.TestCase
    %LEGACYIMAGECURVATUREMEASUREMENTTEST Official wrapper for migrated legacy coverage.

    methods (Test, TestTags = {'Unit'})
        function test_imageCurvatureMeasurement(testCase)
            setupLabKitTestPath();
            legacy_test_imageCurvatureMeasurement();
        end
    end
end

function legacy_test_imageCurvatureMeasurement()
...
function checkCircularFitWithMeasuredScale()
    ...
    opts = struct( ...
        ...
        'doDensify', false, ...
        'denseN', 200);
    fit = curvatureMeasurementWorkflow("computeCurvatureFit", x, y, opts);
    ...
    T = curvatureMeasurementWorkflow("buildCurvatureResultTable", ...
        fit, "sample.png");
...
function checkPixelAndTypedScaleModes()
    ...
    pxOnly = curvatureMeasurementWorkflow("computeCurvatureFit", x, y, ...
    ...
    typedScale = curvatureMeasurementWorkflow("computeCurvatureFit", x, y, ...
...
function checkCurveLengthMeasurement()
    ...
    pxLength = curvatureMeasurementWorkflow( ...
    ...
    mmLength = curvatureMeasurementWorkflow( ...
        "computeCurveLength", x, y, ...
...
function checkDensifyUsesCurvePath()
    ...
    fit = curvatureMeasurementWorkflow( ...
...
function checkInvalidCurvePoints()
    ...
    assertThrows(@() curvatureMeasurementWorkflow( ...
        "computeCurvatureFit", [5; 5; 5], [7; 7; 7], opts), ...
        'labkit_CurvatureMeasurement_app:NotEnoughPoints', ...
        'Duplicate-only curve points should be rejected.');
    assertThrows(@() curvatureMeasurementWorkflow( ...
        "computeCurvatureFit", [1; 2], [3; 4], opts), ...
        'labkit_CurvatureMeasurement_app:NotEnoughPoints', ...
        'Two unique curve points should be rejected.');
    assertThrows(@() curvatureMeasurementWorkflow( ...
        "computeCurveLength", [1], [3], opts), ...
