classdef LegacyFocusStackFusionTest < matlab.unittest.TestCase
    %LEGACYFOCUSSTACKFUSIONTEST Official wrapper for migrated legacy coverage.

    methods (Test, TestTags = {'Unit'})
        function test_focusStackFusion(testCase)
            setupLabKitTestPath();
            legacy_test_focusStackFusion();
        end
    end
end

function legacy_test_focusStackFusion()
...
function checkSyntheticFocusSelection()
    ...
    result = focusStackWorkflow( ...
        "computeFocusStack", {nearImage, farImage}, opts);
...
function checkSummaryTableContract()
    ...
    result = focusStackWorkflow( ...
        "computeFocusStack", {nearImage, farImage}, ...
        struct('focusWindow', 5, 'smoothRadius', 1, 'minConfidence', 0.05));

    T = focusStackWorkflow( ...
        "buildFocusStackSummaryTable", result, ["slice_a.png"; "slice_b.png"]);
...
function checkFolderDiscovery()
    ...
    paths = focusStackWorkflow("findFocusStackImages", folder);
...
function checkSelectedFileSelection()
    ...
    assertThrows(@() focusStackWorkflow( ...
...
function checkRegistrationImprovesSyntheticDrift()
    ...
    [aligned, lines] = focusStackWorkflow( ...
