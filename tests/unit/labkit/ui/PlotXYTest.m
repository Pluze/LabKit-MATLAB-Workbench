classdef PlotXYTest < matlab.unittest.TestCase
    %PLOTXYTEST Verify LabKit behavior through official MATLAB tests.

    methods (Test, TestTags = {'Unit'})
        function test_plotXY(testCase)
            setupLabKitTestPath();
            verify_plotXY();
        end
    end
...
%TEST_PLOTXY Verify prepared X/Y plotting helper behavior.
    ...
    [x, y, xname, yname] = labkit.dta.getCurveXY(curve, 'T', 'Vf');
    ...
    [badX, badY] = labkit.dta.getCurveXY(curve, 't', 'Vf');
    assert(isempty(badX) && isempty(badY), 'getCurveXY should preserve stable exact-case column selection.');
    ...
    assert(abs(lines(1).LineWidth - 1.2) < 1e-12, 'Line width should match stable default.');
