...
%   separate: unknown structure throws rather than widening into a full run.
    ...
    if startsWith(opts.File, "apps/") && numel(parts) >= 5 && ...
        ...
        switch lower(capability)
            ...
            case "workbench"
                targets = [ ...
                    target(opts.SpecsRoot, owner, "presentation", "", "", true, ...
                        "direct workbench presentation behavior in every declared environment"), ...
                    target(opts.SpecsRoot, owner, "workflow", "hidden-gui", "", true, ...
                        "App-owned user journey through the native runtime")];
...
function targets = analysisTargets(specsRoot, appOwner, analysisOwner)
    targets = [ ...
        ...
        target(specsRoot, appOwner + "/workbench", "workflow", "hidden-gui", "", true, ...
            "bounded App user-journey consumer")];
end
