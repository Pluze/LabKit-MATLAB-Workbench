...
%   App definition, and generates its synthetic inputs. It throws on any
%   boundary or contract failure so the caller can aggregate every App result.
    ...
    sample = labkit.app.internal.source.SyntheticInputGenerator.generate( ...
        definition, fullfile(scratchRoot, packageName));
    assert(isa(sample, "labkit.app.synthetic.Pack"), ...
        "LabKit:IsolatedProbe:SyntheticInputs", ...
        "App definition must create a synthetic Pack.");
    if isempty(definition.ProjectSchema)
        assert(isequal(sample.InitialProject, struct()), ...
            "LabKit:IsolatedProbe:SyntheticInputs", ...
            "A project-free App synthetic Pack must use an empty project struct.");
    else
        accepted = definition.ProjectSchema.Validate(sample.InitialProject);
        assert(isequal(accepted, true), ...
            "LabKit:IsolatedProbe:SyntheticInputs", ...
