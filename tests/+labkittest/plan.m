...
function [queries, reasons, fallback] = queriesForChangedPath(path)
    fallback = false;
    parts = split(path, "/");
    if startsWith(path, "apps/") && numel(parts) >= 4 && startsWith(parts(4), "+")
        owner = "apps/" + parts(2) + "/" + parts(3) + "/" + erase(parts(4), "+");
        ...
        if startsWith(fileName, "compute") || startsWith(fileName, "calculate") || ...
            ...
            queries = [query(owner, "scientific", "headless"), ...
                query(owner, "result", "headless"), ...
                query(owner, "presentation", "headless")];
