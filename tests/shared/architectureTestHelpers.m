...
function source = assertAppEntrypoint(root, appName, launchName, legacyCall)
    ...
    appliesVersionDirectly = contains(appSource, '.version()') && ...
    ...
    delegatesVersionToLaunch = ...
        ...
        contains(appSource, '.version');
    assert(appliesVersionDirectly || delegatesVersionToLaunch, ...
    ...
    assert(~contains(appOwnedSource, 'labkit.ui.createTabbedDualPlotShell'), ...
        [appName ' should not use compatibility shell wrappers directly.']);
    forbiddenViewHelpers = {'enablePopout', 'fileSelectionPanel', 'logPanel', ...
        'refreshListboxItems', 'refreshListboxSelection', ...
    ...
    for iHelper = 1:numel(forbiddenViewHelpers)
        ...
        assert(~contains(appOwnedSource, oldViewCall), ...
            [appName ' should not call removed legacy view helper ' oldViewCall '.']);
    end
...
function assertUsesGuiFoundation(source, appName)
    assert(contains(source, 'labkit.ui.runtime.create(') || ...
        ...
        contains(source, 'labkit.ui.runtime.launch('), ...
