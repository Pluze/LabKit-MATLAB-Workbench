...
function source = assertAppEntrypoint(root, appName)
    ...
    forbiddenPackages = {'labkit.io.', 'labkit.data.', ...
        'labkit.analysis.', 'labkit.util.'};
    for k = 1:numel(forbiddenPackages)
        assert(~contains(source, forbiddenPackages{k}), ...
            [appName ' app-owned source uses forbidden package ' ...
            forbiddenPackages{k}]);
    end
end
