...
%TEST_STARTUP_BOUNDARIES Check startup path and root entrypoint boundaries.
    ...
    assert(pathContains(fullfile(root, 'apps', 'electrochem', 'cic')), ...
        'startup_labkit should add nested electrochem app folders to the path.');
    assert(pathContains(fullfile(root, 'apps', 'electrochem', 'vt_resistance')), ...
        'startup_labkit should add nested electrochem app folders to the path.');
    assert(~pathContains(fullfile(root, 'apps', 'electrochem', 'cic', '+cic')), ...
        'startup_labkit should not expose electrochem app-owned package folders as direct path entries.');
    assert(~pathContains(fullfile(root, 'apps', 'electrochem', 'vt_resistance', '+vt_resistance')), ...
        'startup_labkit should not expose electrochem app-owned package folders as direct path entries.');
    ...
    assert(pathContains(fullfile(root, 'apps', 'image_measurement', 'focus_stack')), ...
        'startup_labkit should add nested image measurement app folders to the path.');
    assert(pathContains(fullfile(root, 'apps', 'image_measurement', 'image_enhance')), ...
        'startup_labkit should add nested image measurement app folders to the path.');
    ...
    assert(~pathContains(fullfile(root, 'apps', 'image_measurement', 'batch_crop', 'private')), ...
        'startup_labkit should not expose app-private helper folders as public path entries.');
    assert(~pathContains(fullfile(root, 'apps', 'image_measurement', 'curvature', '+curvature')), ...
        'startup_labkit should not expose app-owned package folders as direct path entries.');
    assert(~pathContains(fullfile(root, 'apps', 'image_measurement', 'focus_stack', '+focus_stack')), ...
        'startup_labkit should not expose app-owned package folders as direct path entries.');
    assert(~pathContains(fullfile(root, 'apps', 'image_measurement', 'image_enhance', '+image_enhance')), ...
        'startup_labkit should not expose app-owned package folders as direct path entries.');
    assert(~pathContains(fullfile(root, 'apps', 'image_measurement', 'image_match', '+image_match')), ...
        'startup_labkit should not expose app-owned package folders as direct path entries.');
    assert(~pathContains(fullfile(root, 'apps', 'image_measurement', 'batch_crop', '+batch_crop')), ...
        'startup_labkit should not expose app-owned package folders as direct path entries.');
