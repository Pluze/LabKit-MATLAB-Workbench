...
function steps = stepsForChangedPath(root, path)
    ...
    if isProjectRoutingPath(path)
        steps = planStep("project", "project", false, ...
            "Reason", "project, docs, or validation-policy file changed");
    elseif first == "+labkit"
    ...
    elseif startsWith(first, ".github") || first == ".agents" || first == "tools"
        steps = planStep("project", "project", false, ...
            "Reason", "agent docs, GitHub workflow, or maintainer tool changed");
...
function steps = labkitPackageSteps(parts)
    if numel(parts) < 2
        steps = [ ...
            ...
            "Reason", "broad LabKit package change needs reusable coverage"), ...
            planStep("gui_apps", "gui/apps", true, ...
            "Reason", "broad LabKit package change can affect app GUI contracts")];
        return;
    end
    ...
    switch packageName
        ...
        otherwise
            steps = [ ...
                planStep("labkit", "labkit", true, ...
                "Reason", "LabKit package change needs reusable coverage"), ...
                planStep("gui_apps", "gui/apps", true, ...
                "Reason", "LabKit package change can affect app GUI contracts")];
    end
end
...
function steps = unitTestPathSteps(parts)
    if numel(parts) >= 5 && parts(4) == "labkit"
        ...
        steps = planStep("labkit_" + safeRunNamePart(area), ...
            "labkit/" + area, false, ...
            "Reason", "LabKit unit test change should rerun the same area");
    ...
    elseif numel(parts) >= 4 && parts(4) == "project"
        steps = planStep("project", "project", false, ...
            "Reason", "project unit test change should rerun project guardrails");
    else
        steps = fullNonGuiStep();
    end
end
...
function steps = fastStepForPlanStep(step)
    ...
    if any(suites == "labkit/ui")
        steps = [ ...
            ...
            "Reason", "fast UI route uses representative UI GUI contracts"), ...
            planStep("gui_labkit_ui_gesture_representative", ...
            "gui/gesture/labkit/ui", true, ...
            "Tests", fastUiGestureRepresentativeTests(), ...
            "Reason", "fast UI route covers representative tool gestures")];
