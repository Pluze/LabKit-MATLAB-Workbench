...
function output = runValidationPlan(root, opts)
    ...
    fprintf("Validation plan routes: %d\n", numel(steps));
    printValidationPlanSteps(steps);

    listings = cell(1, numel(steps));
    for k = 1:numel(steps)
        childArgs = validationPlanStepArgs(steps(k));
        listingOutput = runLabKitTests(childArgs{:}, ...
            "HtmlReport", false, ...
            "FailIfNoTests", opts.FailIfNoTests, ...
            "ArtifactsRoot", opts.ArtifactsRoot, ...
            "RunName", opts.RunName + "_plan_" + steps(k).RunNameSuffix, ...
        ...
        listings{k} = listingOutput.tests;
    end
    [executionSteps, executionSummary] = ...
        labkitCanonicalValidationPlanSteps(steps, listings);
    fprintf("Canonical validation executions: %d tests in %d steps " + ...
        "(%d repeated selections removed)\n", ...
        executionSummary.executionCount, executionSummary.stepCount, ...
        executionSummary.duplicateCount);
    ...
    if opts.ListOnly
        output = struct( ...
            ...
            "count", executionSummary.executionCount, ...
            "runName", opts.RunName, ...
            "listOnly", true, ...
            "tests", canonicalPlanListing(listings, steps));
        return;
    end

    childOutputs = cell(1, numel(executionSteps));
    totalCount = 0;
    for k = 1:numel(executionSteps)
        step = executionSteps(k);
        stepRunName = opts.RunName + "_" + string(k) + "_" + ...
            step.RunNameSuffix;
        fprintf("Validation plan step %d/%d: %s\n", ...
            k, numel(executionSteps), stepRunName);
...
function listing = canonicalPlanListing(listings, steps)
    names = strings(0, 1);
    tags = strings(0, 1);
    seen = strings(1, 0);
    for k = 1:numel(listings)
        current = listings{k};
        currentNames = string(current.Name);
        identities = lower(currentNames) + "|" + string(steps(k).IncludeGui);
        keep = ~ismember(identities, seen);
        names = [names; currentNames(keep)];
        tags = [tags; string(current.Tags(keep))];
