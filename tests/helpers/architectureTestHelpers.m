...
function source = assertAppEntrypoint(root, appName, launchName, legacyCall)
    ...
    assert(contains(appSource, 'labkit.ui.createAppShell'), ...
        [appName ' should build its GUI from the app-facing shell helper.']);
    assert(~contains(appSource, 'labkit.ui.createWorkbench'), ...
        [appName ' should not call deprecated UI shell helpers.']);
    assert(~contains(appSource, 'labkit.ui.handleAppRequest'), ...
        [appName ' should not call deprecated app request helpers.']);
    assert(~contains(appSource, 'labkit.ui.createAppDebugLog'), ...
        [appName ' should not call deprecated debug helpers.']);
    assert(~contains(appSource, 'labkit.ui.createImageAxesRuntime'), ...
        [appName ' should not call deprecated image runtime helpers.']);
...
function assertDICAppBoundary(source, appName)
    ...
    assert(contains(source, 'labkit.ui.createAppShell'), ...
...
function assertAppUsesManagedImageInteractions(source, appName)
    assert(~contains(source, 'WindowScrollWheelFcn'), ...
        [appName ' should register image scroll behavior through labkit.ui.createInteractionRuntime.']);
