    ...
    methods (Test, TestTags = {'Contract:source', 'Env:headless'})
        ...
        function rejectsJournalRootAlongsideAnExplicitJournal(testCase)
            root = testCase.applyFixture( ...
                matlab.unittest.fixtures.TemporaryFolderFixture).Folder;
            definition = runtimeProbeDefinition("run", @runLoggingProbe);
            journal = labkit.app.internal.diagnostics.SessionJournal(definition, RootFolder=root);
            cleanup = onCleanup(@() journal.close());

            testCase.verifyError(@() labkit.app.internal.runtime.RuntimeFactory.createHeadless( ...
...
function state = failLoggingProbe(~, ~)
state = MException("probe:ExpectedFailure", "Expected rollback failure.");
throw(state);
end
...
session = MException("probe:SessionConstructionFailure", ...
    "Intentional session construction failure.");
throw(session);
end
