classdef TestCatalogSpec < matlab.unittest.TestCase
    ...
    methods (Test, TestTags = {'Contract:system', 'Env:headless'})
        ...
        function locateMapsPublicFrameworkFacadeToItsOwner(testCase)
            ...
            nidmm = labkittest.locate("+labkit/+nidmm/readSample.m");
            root = labkittest.setup();
            ...
            testCase.verifyEqual(nidmm.Owner, "labkit/nidmm");
            testCase.verifyEqual(nidmm.Contract, "source");
            testCase.verifyEqual(nidmm.Environment, "headless");
            testCase.verifyEqual(nidmm.Folder, string(fullfile(root, ...
                "tests", "specs", "labkit", "nidmm")));
        end
        ...
        function locateMapsStructuralAppRolesWithoutFileNameHeuristics(testCase)
            ...
            initialState = labkittest.locate( ...
                "apps/electrochem/vt_resistance/+vt_resistance/initialData.m");
        ...
        function ignoredDocumentationPathDoesNotInventAutomatedEvidence(testCase)
            result = labkittest.plan("Profile", "changed", ...
                "ChangedPaths", "docs/develop/testing.md");
