classdef EcgPrintPresentationSpec < matlab.unittest.TestCase
    ...
    methods (Test, TestTags = {'Contract:presentation', 'Env:headless'})
        function buildsWaveformPeaksAndTemplateResidualModels(testCase)
            ...
            cache = struct("signal", working, "workingSignal", working, ...
                "filteredSignal", filtered, "peakDetectionSignal", filtered);
            ...
            spectra = ecg_print.analysisRun.powerSpectraModels(cache);
            testCase.verifyEqual([spectra.axisId], ...
                ["rawSpectrum" "analysisSpectrum" "peakSpectrum"]);
            testCase.verifyTrue(all([spectra.ok]));
            testCase.verifyEqual(spectra(1).frequency(end), 2);
            testCase.verifyEqual(spectra(1).yLabel, ...
                "PSD (dB re 1 mV^2/Hz)");
        end
    end
end
