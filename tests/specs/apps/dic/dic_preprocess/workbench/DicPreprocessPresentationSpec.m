classdef DicPreprocessPresentationSpec < matlab.unittest.TestCase
    ...
    methods (Test, TestTags = {'Contract:presentation', 'Env:headless'})
        ...
        function sourceCanvasAndCommittedCropOwnViewportRevision(testCase)
            sources = struct("id", "source-a");
            model = struct( ...
                "reference", struct("imageData", zeros(8, 10)), ...
                "moving", struct("imageData", zeros(8, 10)));
            original = dic_preprocess.analysisRun.viewportRevision( ...
                sources, model, 3);

            repainted = model;
            repainted.reference.imageData(:) = 1;
            testCase.verifyEqual( ...
                dic_preprocess.analysisRun.viewportRevision( ...
                    sources, repainted, 3), original);

            resized = model;
            resized.moving.imageData = zeros(7, 10);
            testCase.verifyNotEqual( ...
                dic_preprocess.analysisRun.viewportRevision( ...
                    sources, resized, 3), original);
            testCase.verifyNotEqual( ...
                dic_preprocess.analysisRun.viewportRevision( ...
                    struct("id", "source-b"), model, 3), original);
            testCase.verifyNotEqual( ...
                dic_preprocess.analysisRun.viewportRevision( ...
                    sources, model, 4), original);
        end
    end
end
