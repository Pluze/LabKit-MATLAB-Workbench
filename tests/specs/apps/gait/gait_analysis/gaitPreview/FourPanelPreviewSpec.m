classdef FourPanelPreviewSpec < matlab.unittest.TestCase
    ...
    methods (Test, TestTags = {'Contract:source', 'Env:headless'})
        function provesFourPanelPreview(testCase)
            ...
            options.pixelsPerUnit = 2;
            options.unitName = "mm";
            result = gait_analysis.analysisRun.computeGait(pose, options);
            model = struct( ...
                "pose", pose, "result", result, "options", options, ...
                "selectedStep", 1);
            ...
            testCase.verifyEqual(overviewPoint.XData(:), ...
                pose.coords(:, 1, 1) ./ options.pixelsPerUnit, ...
                "AbsTol", 1e-12);
            testCase.verifyEqual(string(axesById.skeleton.XLabel.String), ...
                "X (mm)");
            testCase.verifyEqual(string(axesById.skeleton.YLabel.String), ...
                "Y (mm)");
            testCase.verifyEqual(string(axesById.overview.XLabel.String), ...
                "X (mm)");
            testCase.verifyEqual(string(axesById.overview.YLabel.String), ...
                "Y (mm)");
