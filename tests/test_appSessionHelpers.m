...
%TEST_APPSESSIONHELPERS Verify reusable data/session loading helpers.
    ...
    [session, report2] = gamrywb.data.loadFilesIntoSession(session, {'/tmp/a.DTA', '/tmp/c.DTA'}, @loader, callbacks);
    ...
    [session, report3] = gamrywb.data.loadFilesIntoSession(session, '/tmp/a.DTA', @loader, callbacks);
    ...
    [session, removeReport] = gamrywb.data.removeSelectedItemsFromSession( ...
    ...
    [session, removeReport2] = gamrywb.data.removeSelectedItemsFromSession(session, string.empty(0, 1), removeCallbacks);
    ...
    [allItems, allIdx] = gamrywb.data.selectItemsByNames(session.items, {});
    ...
    [oneItem, oneIdx] = gamrywb.data.selectItemsByNames(session.items, "b.DTA");
    ...
    [noItems, noIdx] = gamrywb.data.selectItemsByNames(session.items, "missing.DTA");
