classdef UiRuntimeKernelTest < matlab.unittest.TestCase
    methods (Test, TestTags = {'Unit'})
        ...
        function taskFileListPreservesRepeatedPortablePaths(testCase)
            setupLabKitTestPath();
            project = labkit.app.project.Schema( ...
                Version=1, Create=@createSourceProject, ...
                Validate=@validateSourceProject);
            layout = labkit.app.layout.workbench({ ...
                labkit.app.layout.fileList("tasks", ...
                    Bind="project.inputs.sources", ...
                    SourceRole="task", SourceIdPrefix="task", ...
                    AllowDuplicatePaths=true)});
            app = labkit.app.Definition( ...
                Entrypoint="labkit_TaskSourceProbe_app", ...
                AppId="probe.task-sources", Title="Task source probe", ...
                Family="Tests", AppVersion="1.0.0", ...
            ...
            testCase.verifyEqual(string({sources.id}), ...
                ["task-1", "task-2"]);
            sourcePaths = string(arrayfun( ...
                @(source) source.reference.originalPath, sources, ...
                "UniformOutput", false));
...
app = labkit.app.Definition( ...
    ...
    "Workbench", labkit.app.layout.workbench({ ...
        ...
        labkit.app.layout.button("increment", "Increment", onPressed, ...
            Tooltip="Increment the probe counter.")}), ...
