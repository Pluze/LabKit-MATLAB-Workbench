...
function checkRequestDispatch()
    ...
    cleaner = onCleanup(@() cleanupFile(debugLogFile));
    ...
    req = labkit.contract.requirements("ui", ">=3.0 <4");
    [handled, outputs, debug] = labkit.ui.app.dispatchRequest( ...
...
function checkRequestErrors()
    ...
    assertThrows(@() labkit.ui.app.dispatchRequest('probe_app', {'__labkit_debug__'}, 0), ...
        'probe_app:UnsupportedInput', 'Internal debug aliases should not be public app input.');
    assertThrows(@() labkit.ui.app.dispatchRequest('probe_app', {'--debug'}, 0), ...
