classdef ParseEISDTATest < matlab.unittest.TestCase
    %PARSEEISDTATEST Verify LabKit behavior through official MATLAB tests.

    methods (Test, TestTags = {'Unit'})
        function test_parseEISDTA(testCase)
            setupLabKitTestPath();
            verify_parseEISDTA();
        end
    end
...
%TEST_PARSEEISDTA Verify extracted EIS parser and ZCURVE accessors.
    ...
    assert(strcmp(meta.tag, ''), 'Two-token TAG metadata should preserve stable empty-tag behavior.');
    ...
    assert(ok, msg);
    assert(strcmp(msg, 'Using table: ZCURVE'), 'ZCURVE message should match stable wording.');

    freq = labkit.dta.getColumn(curve, 'Freq');
    zreal = labkit.dta.getColumn(curve, 'Zreal');
    zimag = labkit.dta.getColumn(curve, 'zimag');

    assert(numel(freq) == 3, 'EIS fixture should contain the synthetic impedance points.');
