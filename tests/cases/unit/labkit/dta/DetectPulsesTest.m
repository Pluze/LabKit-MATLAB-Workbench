classdef DetectPulsesTest < matlab.unittest.TestCase
    %DETECTPULSESTEST Verify LabKit behavior through official MATLAB tests.

    methods (Test, TestTags = {'Unit'})
        function test_detectPulses(testCase)
            setupLabKitTestPath();
            verify_detectPulses();
        end
    end
...
%TEST_DETECTPULSES Verify extracted pulse detection behavior.
    ...
    [currentCurve, currentOk, currentMsg] = labkit.dta.getMainCurve(currentTables);
    assert(currentOk, currentMsg);
    currentT = labkit.dta.getColumn(currentCurve, 'T');
    currentIm = labkit.dta.getColumn(currentCurve, 'Im');

    [currentPulse, currentPulseMsg] = labkit.dta.detectPulses(currentT, currentIm, currentMeta);
    ...
    assert(abs(currentPulse.cath_start - 1) < 1e-12, 'Current fixture cathodic start should match TSTEP accumulation.');
    assert(abs(currentPulse.cath_end - 2) < 1e-12, 'Current fixture cathodic end should match TSTEP accumulation.');
    assert(abs(currentPulse.gap_start - 2) < 1e-12 && abs(currentPulse.gap_end - 4) < 1e-12, ...
        'Current fixture blank gap should include both zero-current steps.');
    assert(abs(currentPulse.gap.center_s - 3) < 1e-12, 'Current fixture normalized gap center should be populated.');

    [voltageItem, voltageStatus] = labkit.dta.loadFile(voltageFixture, "chrono");
    assert(voltageStatus.ok, voltageStatus.message);
    voltageMeta = voltageItem.meta;
    voltageTables = voltageItem.tables;
    [voltageCurve, voltageOk, voltageMsg] = labkit.dta.getMainCurve(voltageTables);
    assert(voltageOk, voltageMsg);
    voltageT = labkit.dta.getColumn(voltageCurve, 'T');
    voltageIm = labkit.dta.getColumn(voltageCurve, 'Im');
    [voltagePulse, voltagePulseMsg] = labkit.dta.detectPulses(voltageT, voltageIm, voltageMeta);
    ...
    assert(abs(voltagePulse.anod_start - 4) < 1e-12, 'Voltage fixture anodic start should match VSTEP timing.');
    ...
    [pulse, msg] = labkit.dta.detectPulses(t, Im, meta);
    assert(pulse.ok, msg);
    ...
    assert(strcmp(msg, 'Metadata pulse detection OK (current-controlled): cath step 1, anod step 3.'), ...
        'Metadata detection message should match stable wording.');
    ...
    assert(abs(pulse.gap_start - 0.10) < 1e-12 && abs(pulse.gap_end - 0.14) < 1e-12, ...
        'Metadata gap timing should match stable fields.');
    ...
    [uiDefaultPulse, uiDefaultMsg] = labkit.dta.detectPulses(t, Im, meta, "Metadata first, then auto");
    ...
    [autoPulse, autoMsg] = labkit.dta.detectPulses(t, Im, meta, opts);
    ...
    [uiAutoPulse, uiAutoMsg] = labkit.dta.detectPulses(t, Im, meta, "Auto from Im only");
    ...
    [fallbackPulse, fallbackMsg] = labkit.dta.detectPulses(t, Im, badMeta);
    ...
    [metadataOnlyPulse, metadataOnlyMsg] = labkit.dta.detectPulses(t, Im, badMeta, "Metadata only");
    ...
    assert(strcmp(metadataOnlyMsg, 'Metadata pulse detection: no ISTEP/TSTEP or VSTEP/TSTEP steps found.'), ...
        'Metadata-only failure message should match stable wording.');
end
