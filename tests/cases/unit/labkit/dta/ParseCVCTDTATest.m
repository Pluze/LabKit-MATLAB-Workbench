classdef ParseCVCTDTATest < matlab.unittest.TestCase
    %PARSECVCTDTATEST Verify LabKit behavior through official MATLAB tests.

    methods (Test, TestTags = {'Unit'})
        function test_parseCVCTDTA(testCase)
            setupLabKitTestPath();
            verify_parseCVCTDTA();
        end
    end
...
%TEST_PARSECVCTDTA Verify extracted CV/CT parser behavior.
    ...
    assert(abs(demoScanRate - 0.2) < 1e-12, 'Demo SCANRATE should convert from mV/s to V/s.');
