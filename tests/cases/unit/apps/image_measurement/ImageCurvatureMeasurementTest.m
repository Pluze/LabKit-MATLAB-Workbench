...
function checkCircularFitWithMeasuredScale()
    ...
    opts = struct( ...
        ...
        'doDensify', false, ...
        'denseN', 200);
    fit = curvature.analysisRun.computeFitFromOptions(x, y, opts);
    ...
    T = curvature.resultFiles.buildResultTable(fit, "sample.png");
...
function checkPixelAndTypedScaleModes()
    ...
    pxOnly = curvature.analysisRun.computeFitFromOptions(x, y, ...
    ...
    typedScale = curvature.analysisRun.computeFitFromOptions(x, y, ...
...
function checkCurveLengthMeasurement()
    ...
    pxLength = curvature.analysisRun.computeLengthFromOptions(x, y, ...
        struct('referencePx', NaN, 'referenceLength', 0, 'scaleUnit', 'um'));
    ...
    mmLength = curvature.analysisRun.computeLengthFromOptions(x, y, ...
...
function checkDensifyUsesCurvePath()
    ...
    fit = curvature.analysisRun.computeFitFromOptions(anchorX, anchorY, ...
...
function checkTaskFingerprintsTrackInputs()
    ...
    fitTask = curvature.appState.fitTask(points, fitPath, calibration, ...
        struct('doDensify', true, 'denseN', 25));
    sameFitTask = curvature.appState.fitTask(points, fitPath, calibration, ...
        struct('doDensify', true, 'denseN', 25));
    changedFitTask = curvature.appState.fitTask(points, fitPath, calibration, ...
    ...
    lengthTask = curvature.appState.lengthTask(points, fitPath, calibration);
    changedLengthTask = curvature.appState.lengthTask(points + [0 1], fitPath, calibration);
    changedPathTask = curvature.appState.lengthTask(points, points, calibration);
...
function checkInvalidCurvePoints()
    ...
    assertThrows(@() curvature.analysisRun.computeFitFromOptions( ...
        [5; 5; 5], [7; 7; 7], opts), ...
