...
function checkRuntimeV2ProjectAndPresenterContracts()
    ...
    assert(definition.product.version == "1.6.4", ...
        'Image Match definition should own product metadata.');
...
function checkManifestAndExportContract()
    ...
    assert(isscalar(payload.results), ...
...
function checkExportTaskFingerprintTracksReferenceOptionsAndSteps()
    ...
    referenceItem = image_match.sourceFiles.emptyItem();
    ...
    base = image_match.resultFiles.exportTask(item, referenceItem, step, struct( ...
        'outputFolder', "out_a", ...
        'format', 'PNG'));
    repeated = image_match.resultFiles.exportTask( ...
        item, referenceItem, step, struct( ...
        'outputFolder', "out_a", ...
        'format', 'PNG'));
    changedReference = referenceItem;
    changedReference.path = "reference_b.png";
    changedReferenceTask = image_match.resultFiles.exportTask( ...
        item, changedReference, ...
        step, struct('outputFolder', "out_a", 'format', 'PNG'));
    changedStep = image_match.resultFiles.exportTask(item, referenceItem, ...
