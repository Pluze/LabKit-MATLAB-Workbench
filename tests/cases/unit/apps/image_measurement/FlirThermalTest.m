    ...
    methods (Test, TestTags = {'Unit'})
        function flirThermalAppReadsSummarizesAndExports(testCase)
            ...
            testCase.verifyTrue(ismember('manualPointSet', ...
                payload.manifest.Properties.VariableNames));
            testCase.verifyTrue(ismember('gammaValue', ...
                payload.manifest.Properties.VariableNames));
            testCase.verifyTrue(isnan(payload.manifest.manualTempC(1)));
            testCase.verifyEqual(payload.manifest.gammaValue(1), 1.6, ...
                "AbsTol", 1e-12);
            ...
            testCase.verifyGreaterThan(max(abs(gammaRgb(:) - strongerGammaRgb(:))), 0, ...
                'Gamma value should adjust rendered RGB colors.');
