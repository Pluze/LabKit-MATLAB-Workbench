...
%TEST_BATCHIMAGECROP Verify batch microscope crop calculations and exports.
    ...
    checkAppSdkProjectAndScaleBarGeometry();
end

function checkAppSdkProjectAndScaleBarGeometry()
    definition = batch_crop.definition();
    assert(definition.AppId == "batch_crop", ...
        'Batch crop definition should use the explicit App SDK contract.');
    project = definition.ProjectSchema.Create();
    assert(definition.ProjectSchema.Version == 3 && ...
        isa(definition.ProjectSchema.Migrate, 'function_handle'), ...
        ['Batch crop should migrate older projects to distinct ' ...
         'task source records.']);
    ...
    legacyProject.inputs.items = [item; item];
    migratedV2 = definition.ProjectSchema.Migrate(legacyProject, 1);
    migrated = definition.ProjectSchema.Migrate(migratedV2, 2);
    assert(~isfield(migrated.inputs.items, 'image') && ...
        ~isfield(migrated.inputs.items, 'path') && ...
        numel(migrated.inputs.sources) == 2 && ...
        numel(unique(string({migrated.inputs.sources.id}))) == 2 && ...
        isequal(string({migrated.inputs.items.sourceId}), ...
            string({migrated.inputs.sources.id})) && ...
        all(string({migrated.inputs.sources.role}) == "cropSource"), ...
    ...
    sourcePath = string(tempname) + ".png";
    imwrite(item.image, sourcePath);
    cleanup = onCleanup(@() deleteIfPresent(sourcePath));
...
function checkPreviewViewPreservesZoomAcrossPaddingRedraw()
    ...
    state = batch_crop.cropPreview.captureView(ax, geometry, placement);
    ax.XLim = placement.xData + [-0.5, 0.5];
    ax.YLim = placement.yData + [-0.5, 0.5];
    batch_crop.cropPreview.restoreView(ax, state, geometry, placement);
...
function checkPreviewViewPreservesOriginalRoiAcrossPreviewScaleChanges()
    ...
    geometryA = batch_crop.cropGeometry.prepareCropCanvas(img, struct( ...
        'angleDeg', 0, ...
        'paddingPercent', 20));
    placementA = batch_crop.cropPreview.placement(geometryA);
    geometryB = batch_crop.cropGeometry.prepareCropCanvas(img, struct( ...
        'angleDeg', 0, ...
        'paddingPercent', 200, ...
        'maxCanvasPixels', 5000));
    placementB = batch_crop.cropPreview.placement(geometryB);
    ...
    state = batch_crop.cropPreview.captureView(ax, geometryA, placementA);
    batch_crop.cropPreview.restoreView(ax, state, geometryB, placementB);
...
function checkDuplicateItemCreatesIndependentCropTask()
    ...
    item.scaleCalibration = labkit.app.interaction.scaleCalibration(80, 20, "um", ...
