...
function checkExplicitUiContract()
    ...
    assert(isa(app, "labkit.ui.Application"), ...
        'Chrono overlay should use the explicit Application contract.');
    project = app.Project.Create();
    assert(app.Project.Validate(project), ...
        'The default Chrono overlay project should validate.');
    assert(app.Project.Version == 2 && ...
        isa(app.Project.Migrate, 'function_handle'), ...
        'Payload version 2 should expose one version-aware migration entry.');
    assert(isequal(app.commandIdsForRuntime(), "exportCSV"), ...
        'Only the Chrono-owned export behavior should require a Command.');
    version = labkit_ChronoOverlay_app("version");
    assert(string(version.version) == "1.5.0", ...
        'The entrypoint version request should use Application metadata.');

    invalid = project;
    invalid.parameters.lineWidth = Inf;
    assert(~app.Project.Validate(invalid), ...
    ...
    migrated = app.Project.Migrate(legacyProject, 1);
    ...
    session = struct( ...
        ...
        "selection", struct("files", ...
            labkit.ui.Selection(Indices=1)));
    state = struct('project', project, 'session', session);
    presentation = chrono_overlay.userInterface.presentWorkbench(state);
    assert(isa(presentation, "labkit.ui.Presentation"), ...
        'The presenter should return the closed Presentation value.');
end
