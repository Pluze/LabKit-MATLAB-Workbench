...
%TEST_VTRESISTANCEEXPORT Verify VT resistance result/export table helpers.
    ...
    assert(definition.ProjectSchema.Version == 1 && ...
        isempty(definition.ProjectSchema.Migrate), ...
        'VT Resistance should use a first-version App SDK project.');
    ...
    session = definition.CreateSession(project, context);
    assert(isempty(session.selection.files.Indices) && ...
...
function C = buildVTBatchTableData(items)
    C = vt_resistance.analysisRun.buildBatchTableData(items);
end
