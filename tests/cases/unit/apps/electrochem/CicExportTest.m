...
%TEST_CICEXPORT Verify app-side CIC result/export table helpers.
    ...
    assert(isa(definition, "labkit.app.Definition") && ...
        definition.ProjectSchema.Version == 1 && ...
        isempty(definition.ProjectSchema.Migrate), ...
        'CIC should use a first-version App SDK project contract.');
    ...
    presentation = definition.PresentWorkbench(state);
    assert(isa(presentation, "labkit.app.view.Snapshot"), ...
        'CIC presenter should return an App SDK snapshot.');
...
function [C, cols] = buildCICBatchTableData(items, unitLabel)
    [C, cols] = cic.analysisRun.buildBatchTableData(items, unitLabel);
end
