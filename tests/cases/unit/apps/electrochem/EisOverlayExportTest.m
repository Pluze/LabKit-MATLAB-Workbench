...
%TEST_EISOVERLAYEXPORT Verify EIS item schema and export/plot contracts.
    ...
    zreal = eis.analysisRun.valuesForAxis(canonicalItem, 'Zreal (ohm)');
    assertClose(zreal, item.Zreal_ohm, 'EIS app axis-value hook should preserve Zreal values');
    logFreq = eis.analysisRun.valuesForAxis(canonicalItem, 'log10(Freq)');
    ...
    summary = eis.userInterface.buildSummary(canonicalItem);
    ...
    assert(eis.userInterface.axisModeForSelection('Zreal (ohm)', '-Zimag (ohm)') == "equal", ...
        'Nyquist EIS selection should use equal axes.');
    assert(eis.userInterface.axisModeForSelection('Zreal (ohm)', '-Zimag (ohm)', true, false) == "normal", ...
        'Log-scaled Nyquist EIS selection should release equal axes for stable zooming.');
    assert(eis.userInterface.axisModeForSelection('Zreal (ohm)', '-Zimag (ohm)', false, true) == "normal", ...
        'Log-scaled Nyquist EIS selection should release equal axes for stable zooming.');
    assert(eis.userInterface.axisModeForSelection('Freq (Hz)', 'Zmod (ohm)') == "normal", ...
        'Non-Nyquist EIS selection should use normal axes.');
end
