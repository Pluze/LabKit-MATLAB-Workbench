...
%TEST_EISOVERLAYEXPORT Verify EIS item schema and export/plot contracts.
    ...
    summary = eis.view.buildSummary(canonicalItem);
    ...
    assert(eis.view.axisModeForSelection('Zreal (ohm)', '-Zimag (ohm)') == "equal", ...
        'Nyquist EIS selection should use equal axes.');
    assert(eis.view.axisModeForSelection('Zreal (ohm)', '-Zimag (ohm)', true, false) == "normal", ...
        'Log-scaled Nyquist EIS selection should release equal axes for stable zooming.');
    assert(eis.view.axisModeForSelection('Zreal (ohm)', '-Zimag (ohm)', false, true) == "normal", ...
        'Log-scaled Nyquist EIS selection should release equal axes for stable zooming.');
    assert(eis.view.axisModeForSelection('Freq (Hz)', 'Zmod (ohm)') == "normal", ...
