...
%TEST_EISOVERLAYEXPORT Verify EIS item schema and export/plot contracts.
    ...
    assert(definition.contractVersion == 2 && ...
        ...
        'EIS should use a first-version Runtime V2 project contract.');
    ...
    assert(isfield(presentation.previews.plot.Axes, 'overlay') && ...
        string(presentation.controls.files.Selection) == "item1", ...
        'EIS presenter should prepare its overlay and source selection.');
    ...
    assert(eis.userInterface.axisModeForSelection( ...
        axisItems(5), axisItems(7)) == "equal", ...
        'Nyquist EIS selection should use equal axes.');
    assert(eis.userInterface.axisModeForSelection( ...
        axisItems(5), axisItems(7), true, false) == "normal", ...
        'Log-scaled Nyquist EIS selection should release equal axes for stable zooming.');
    assert(eis.userInterface.axisModeForSelection( ...
        axisItems(5), axisItems(7), false, true) == "normal", ...
        'Log-scaled Nyquist EIS selection should release equal axes for stable zooming.');
    assert(eis.userInterface.axisModeForSelection( ...
        axisItems(1), axisItems(8)) == "normal", ...
