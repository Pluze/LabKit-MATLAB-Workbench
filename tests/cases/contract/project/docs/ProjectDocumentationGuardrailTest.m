classdef ProjectDocumentationGuardrailTest < matlab.unittest.TestCase
    ...
    methods (Test, TestTags = {'Integration', 'Style'})
        ...
        function zeroDebtMigrationLedgerContainsNoCompletedRoadmap(testCase)
            root = setupLabKitTestPath();
            ledger = string(fileread(fullfile(root, ".agents", ...
                "migration_guide.md")));

            ledgerIsEmpty = contains(ledger, ...
                "toolbox-product-debt: none") && contains(ledger, ...
                "architecture-migration-debt: none");
            testCase.assumeTrue(ledgerIsEmpty, ...
                "This contract applies only when every migration debt category is empty.");
            retiredSections = [ ...
                "## Working order", ...
                "## Defect closure gate", ...
                "## Maturity work after closure"];
            for k = 1:numel(retiredSections)
                testCase.verifyFalse(contains(ledger, retiredSections(k)), ...
                    ["A zero-debt ledger must not retain a completed roadmap: " ...
                    retiredSections(k)]);
            end
        end
