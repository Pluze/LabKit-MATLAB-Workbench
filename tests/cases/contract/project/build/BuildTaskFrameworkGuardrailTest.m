classdef BuildTaskFrameworkGuardrailTest < matlab.unittest.TestCase
    ...
    methods (Test, TestTags = {'Integration', 'Style'})
        ...
        function internalShardsRelayStructuredProgress(testCase)
            root = setupLabKitTestPath();
            buildSource = string(fileread(fullfile(root, "buildfile.m")));
            workerSource = string(fileread(fullfile(root, "tests", ...
                "runner", "labkitRunInternalShards.m")));
            progressSource = string(fileread(fullfile(root, "tests", ...
                "runner", "labkitProgressPlugin.m")));

            testCase.verifyTrue(contains(buildSource, ...
                "labkitRunInternalShards(root, spec, args)"), ...
                "The buildfile should delegate worker orchestration.");
            testCase.verifyTrue(contains(workerSource, "active-test.json") && ...
                contains(workerSource, "active-test.txt") && ...
                contains(workerSource, "LabKit internal shard status:"));
            testCase.verifyTrue(contains(workerSource, ...
                'javaObject("java.lang.ProcessBuilder"'));
            testCase.verifyFalse(any(contains(workerSource, ...
                ["#!/bin/sh", "run_shards.sh", "shellQuote("])), ...
                "Worker orchestration should not depend on a platform shell.");
            testCase.verifyFalse(contains(buildSource, "|| ispc"), ...
                "Windows should use the same local worker wrapper.");
            testCase.verifyTrue(contains(progressSource, ...
                "eta=%s test_elapsed=%s"), ...
                "Each shard summary should include its own ETA.");
        end
