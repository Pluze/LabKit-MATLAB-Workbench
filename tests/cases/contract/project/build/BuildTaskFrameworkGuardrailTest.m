classdef BuildTaskFrameworkGuardrailTest < matlab.unittest.TestCase
    ...
    methods (Test, TestTags = {'Integration', 'Style'})
        ...
        function kindPrefixedSuiteDiscoversFocusedMethodSelector(testCase)
            setupLabKitTestPath();
            output = listLabKitTestsQuietly( ...
                "Suites", "contract/apps/gait/gait_analysis/isolatedPath", ...
                "Tests", ...
                "runsWithoutSiblingApps", ...
                "RunName", "kind_prefixed_selector_probe");
            testCase.verifyEqual(output.count, 1);
            testCase.verifyEqual(output.tests.Name, ...
                "GaitAnalysisIsolatedPathContractTest/runsWithoutSiblingApps");
        end

        function runnerFilesRequiresGuiModeForGuiFiles(testCase)
            root = setupLabKitTestPath();
            guiFile = fullfile(root, "tests", "cases", "gui", ...
                "labkit_framework", "ui", "UiMatlabPlatformAdapterTest.m");
            testCase.verifyError(@() runLabKitTests( ...
        ...
        function focusedRunnerCanSkipHtmlReport(testCase)
            ...
            testCase.verifyTrue(isfile(output.artifacts.activeTestText), ...
                "Focused runner should preserve a human-readable active-test summary.");
            activeSummary = strip(string( ...
                fileread(output.artifacts.activeTestText)));
            testCase.verifyMatches(activeSummary, ...
                "^DONE 1/1 elapsed=[0-9]{2}:[0-9]{2}:[0-9]{2} " + ...
                "eta=00:00:00$");
            ...
            heartbeatTag = "labkitProgressHeartbeat:" + ...
                string(output.artifacts.activeTest);
            testCase.verifyEmpty(timerfindall("Tag", char(heartbeatTag)), ...
                "Focused runner must release its heartbeat timer before returning.");
        ...
        function discoveryProbeCanSuppressLongListings(testCase)
            root = setupLabKitTestPath();
            file = fullfile(root, "tests", "cases", "unit", ...
                "project", "PlatformSkeletonTest.m");
            text = evalc(['output = runLabKitTests("Files", file, ' ...
                '"ListOnly", true, "PrintList", false);']);

            testCase.verifyGreaterThan(output.count, 0);
            testCase.verifyFalse(contains(text, ...
                "Matched official tests:"));
        end

        function runnerPublishesStructuredProgressAndEta(testCase)
            root = setupLabKitTestPath();
            progressSource = string(fileread(fullfile(root, "tests", ...
                "runner", "labkitProgressPlugin.m")));

            testCase.verifyTrue(contains(progressSource, ...
                "test-progress.jsonl") && ...
                contains(progressSource, "active-test.json") && ...
                contains(progressSource, "active-test.txt"));
            testCase.verifyTrue(contains(progressSource, ...
                "eta=%s test_elapsed=%s"), ...
                "Progress should include ETA and active-test elapsed time.");
        end
        ...
        function affectedValidationMapperCoversSharedUiAndAppChanges(testCase)
            ...
            steps = labkitValidationPlanForChangedPaths(root, [ ...
                "+labkit/+app/+internal/@MatlabPlatformAdapter/applyView.m", ...
            ...
            testCase.verifyTrue(any(signatures == ...
                "unit/apps/image_measurement/batch_crop/appContract|false"));
            testCase.verifyTrue(any(signatures == "gui/apps|true"));
        end
        ...
        function affectedValidationMapperCoversImageFacadeChanges(testCase)
            ...
            testCase.verifyTrue(any(signatures == ...
                "contract/apps/image_measurement/batch_crop/isolatedPath|false"));
            testCase.verifyTrue(any(signatures == ...
                "gui/apps/image_measurement/batch_crop/smoke|true"));
            testCase.verifyFalse(any(signatures == "apps/image_measurement|false"));
        end
        ...
        function testFilesUseKnownTags(testCase)
            ...
            allowedTags = ["Unit", "Integration", "GUI", "Structural", ...
                ...
                "RouteFeature:app-layout", "RouteFeature:axes-presentation"];
