classdef BuildTaskEfficiencyGuardrailTest < matlab.unittest.TestCase
    ...
    methods (Test, TestTags = {'Integration', 'Style'})
        ...
        function changedAppCapabilityUsesExplicitTestKinds(testCase)
            ...
            testCase.verifyTrue(any(signatures == "unit/apps/labkit_core|false"), ...
                "An App capability must select an explicit unit suite, even while the old tree requires a family fallback.");
            testCase.verifyTrue(any(signatures == ...
                "gui/apps/labkit_core/figure_studio|true"), ...
                "An App capability must select its explicit App GUI suite, not a semantic suite whose kind is inferred later.");
            testCase.verifyTrue(any(signatures == "contract/apps|false"), ...
                "The isolated-path contract remains an explicit contract selection.");
            testCase.verifyTrue(any(contains(reasons, "migration fallback")), ...
                "The planner must expose an unmigrated owner scope instead of silently widening it.");
        end
