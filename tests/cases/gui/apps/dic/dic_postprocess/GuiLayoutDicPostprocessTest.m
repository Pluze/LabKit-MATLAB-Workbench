classdef GuiLayoutDicPostprocessTest < matlab.unittest.TestCase
    ...
    methods (Test, TestTags = {'GUI', 'Structural', 'Workflow'})
        function dic_postprocess_workflow_generates_overlays_and_summary(testCase)
            ...
            [generated, waitInfo] = h.waitForCondition(fig, ...
            ...
            testCase.verifyTrue(generated, sprintf(['DIC postprocess workflow should ' ...
                'generate overlays and summary. %s'], ...
            ...
            testCase.verifyEqual(runtime.definition.contractVersion, 2, ...
                'DIC postprocess workflow must execute through runtime V2.');
            runtime.request.outputFolderChooser = @(~, ~) folder;
            runtime.request.outputFileChooser = @(~, ~, ~) deal( ...
                'synthetic_dic_strain_summary.csv', folder);
            setappdata(fig, 'labkitUiAppRuntime', runtime);
            driver.click('Save overlay PNGs');
            driver.click('Export summary CSV');
            expectedOutputs = { ...
            ...
            testCase.verifyGreaterThan(size(driver.tableData('resultTable'), 1), 0, ...
                'Project reopen should restore the durable summary.');
            testCase.verifyGreaterThan( ...
                numel(ui.controls.overlayAxes.axesById.exx.Children), 0, ...
                'Project reopen should rebuild the EXX overlay cache.');
            testCase.verifyGreaterThan( ...
                numel(ui.controls.overlayAxes.axesById.eyy.Children), 0, ...
                'Project reopen should rebuild the EYY overlay cache.');
        end
    end
end
