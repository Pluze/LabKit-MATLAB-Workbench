...
%TEST_GUI_LAYOUT_UI_SCALE_BAR_PANEL Verify reusable scale-bar panel contracts.
    ...
    ui = labkit.ui.interaction.scaleBar(grid, 2, runtime, ...
    ...
    geometry = ui.scaleBarGeometry();
    assert(isequal(geometry.color, [0 0 0]) && strcmp(geometry.colorName, 'Black'), ...
        'Default scale-bar color should be black.');
    assert(strcmp(geometry.label, '1 m') && geometry.pixelsPerUnit == 4, ...
        'Scale-bar geometry should expose a real-unit label and calibration.');
    assert(size(geometry.line, 1) == 2 && size(geometry.line, 2) == 2, ...
        'Scale-bar geometry should return a two-endpoint line.');
    ...
    h.waitForUiIdle(fig);
    whiteGeometry = ui.scaleBarGeometry();
    assert(isequal(whiteGeometry.color, [1 1 1]) && strcmp(whiteGeometry.colorName, 'White'), ...
        'Scale-bar panel should map the White option to a white drawing color.');
    assert(strcmp(whiteGeometry.position, 'Top left') && strcmp(whiteGeometry.verticalAlignment, 'top'), ...
