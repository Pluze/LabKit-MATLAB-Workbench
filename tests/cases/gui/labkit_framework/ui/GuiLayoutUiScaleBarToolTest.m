classdef GuiLayoutUiScaleBarToolTest < matlab.unittest.TestCase
...
%TEST_GUI_LAYOUT_UI_SCALE_BAR_TOOL Verify high-level scale-bar tool contracts.
    ...
    tool = labkit.ui.interaction.scaleBar(grid, 2, runtime, ...
    ...
    restored = labkit.ui.interaction.scaleBarCalibration(150, 30, "um", ...
    ...
    h.waitForUiIdle(fig);
    geometry = tool.scaleBarGeometry();
    assert(strcmp(geometry.label, '10 mm') && geometry.pixelsPerUnit == 4, ...
        'Scale-bar tool should build the current scale-bar geometry.');

    h.invokeCallback(tool.controls.placeButton, 'ButtonPushedFcn');
    assert(tool.hasScaleBar() && calls.placed == 1, ...
        'Place button should store a placed scale-bar geometry.');
    ...
    tool2 = labkit.ui.interaction.scaleBar(grid2, 2, runtime2, ...
