...
%TEST_GUI_LAYOUT_UI_AXES_WORKBENCH Verify declarative workbench axes behavior.
    ...
    assert(numel(ui.main.RowHeight) == 3 && isequal(ui.main.RowHeight{1}, 30), ...
        'UI 4.x workbench shell should reserve a compact utility bar row.');
    ...
    assert(~isempty(utilityBar) && ~isempty(utilitySavePlot), ...
        'Workbench shell should expose stable utility bar controls.');
    utilityPlotPath = string(tempname) + ".png";
    setappdata(ui.figure, 'labkitUiUtilityPlotFile', utilityPlotPath);
    h.invokeCallback(utilitySavePlot(1), 'ButtonPushedFcn');
    assert(isfile(utilityPlotPath), ...
        'Utility bar should export the current plot through MATLAB graphics export.');
    ...
    toolbar = findall(popoutFig(1), 'Type', 'uitoolbar', ...
        'Tag', 'labkitAxesPopoutToolbar');
    assert(~isempty(toolbar), ...
        'Enhanced axes popout should install copied-figure utility tools.');
    ...
    dataTool = findall(popoutFig(1), 'Tag', 'labkitAxesPopoutDataTool');
    assert(~isempty(fontTool) && ~isempty(lineTool) && ~isempty(dataTool), ...
        'Enhanced axes popout should expose stable tool tags.');
    sourceFontSize = plotAx.FontSize;
    copiedFontSize = popoutAxes(1).FontSize;
    h.invokeCallback(fontTool(1), 'ClickedCallback');
    ...
    h.invokeCallback(lineTool(1), 'ClickedCallback');
    ...
    setappdata(popoutFig(1), 'labkitAxesPopoutExportFolder', exportFolder);
    h.invokeCallback(dataTool(1), 'ClickedCallback');
    assert(isfile(fullfile(exportFolder, "plot_data.mat")) && ...
        isfile(fullfile(exportFolder, "plot_data.csv")) && ...
        isfile(fullfile(exportFolder, "recreate_plot.m")) && ...
        isfile(fullfile(exportFolder, "README.txt")), ...
        'Line plot data export should write MAT, CSV, script, and README files.');
    exported = load(fullfile(exportFolder, "plot_data.mat"), "plotData");
    assert(exported.plotData.schema == "labkit.ui.tool.axesData.v1" && ...
        ~isempty(exported.plotData.objects), ...
        'Plot data export should store the LabKit axes data schema and objects.');
