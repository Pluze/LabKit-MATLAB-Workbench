classdef GuiLayoutUiAxesWorkbenchTest < matlab.uitest.TestCase
    %GUILAYOUTUIAXESWORKBENCHTEST Verify UI 4.0 shell and axes behavior.
...
%TEST_GUI_LAYOUT_UI_AXES_WORKBENCH Verify declarative workbench axes behavior.
    ...
    assert(isequal(ui.main.ColumnWidth, {420, 6, '1x'}), ...
        'UI 4.0 app builder should create the standard resizable workbench layout.');
    assert(numel(ui.main.RowHeight) == 3 && isequal(ui.main.RowHeight{1}, 0), ...
        'UI 4.x workbench shell should put utility actions in native window menus.');
    assert(strcmp(ui.rightPanel.Title, 'Preview'), ...
        'UI 4.0 app builder should preserve the workspace title.');
    ...
    plotAx.XLim = [0 10];
    plotAx.YLim = [0 20];
    didZoomTime = labkit.ui.tool.zoomAxesAtPoint(plotAx, [5 10], -1);
    ...
    h.assertAxesPopoutEnabled(plotAx, ...
        'UI 4.0 preview axes should install the LabKit popout context action.');
    ...
    hSecondImage = labkit.ui.view.drawImage(ui, 'plotPreview', ...
