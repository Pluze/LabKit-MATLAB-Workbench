classdef UiMatlabPlatformAdapterTest < matlab.unittest.TestCase
    methods (Test, TestTags = {'GUI', 'Structural'})
        ...
        function acceptsRendererViewportWhenViewRevisionChanges(testCase)
            setupLabKitTestPath();
            helpers = guiTestHelpers();
            helpers.assertUifigureAvailable();
            ...
            cleanup = onCleanup(@() runtime.close());
            figure = runtime.figureHandle();
            ax = component(figure, "preview.main");
            ax.XLim = [-1 4];
            ax.YLim = [-2 3];

            reset = component(figure, "resetView");
            reset.ButtonPushedFcn(reset, struct());
            drawnow;

            testCase.verifyEqual(ax.XLim, [0 10]);
            testCase.verifyEqual(ax.YLim, [0 20]);
            clear cleanup
        end
        ...
        function enforcesSharedWorkbenchVisualPolicy(testCase)
            ...
            testCase.verifyEqual(string(choice.Enable), "off");
            testCase.verifyEqual(string(component( ...
                figure, "visualChoice.label").Enable), "off");
            ...
            testCase.verifyEqual(string(autoThird.Tooltip), ...
                "Explain the third scientific action.");
            testCase.verifyFalse(contains(string(autoThird.Tooltip), newline));

            visualFileChoose = component(figure, "visualFile.choose");
            testCase.verifyEqual(string(visualFileChoose.Tooltip), ...
                "Choose one calibrated probe image.");
            testCase.verifyEqual(numel(findall(figure, ...
            ...
            menuTags = ["labkitAppUtilityToolsMenu", ...
                "labkitAppUtilityPlotMenu", ...
                "labkitAppUtilityScreenshotMenu", ...
                "labkitAppUtilityCopyScreenshot", ...
                "labkitAppUtilityScreenshot"];
            for tag = menuTags
                testCase.verifyEqual(numel(findall( ...
                    figure, "Tag", tag)), 1);
            end
            toolsMenu = findall(figure, ...
                "Tag", "labkitAppUtilityToolsMenu");
            plotMenu = findall(figure, ...
                "Tag", "labkitAppUtilityPlotMenu");
            screenshotMenu = findall(figure, ...
                "Tag", "labkitAppUtilityScreenshotMenu");
            copyScreenshot = findall(figure, ...
                "Tag", "labkitAppUtilityCopyScreenshot");
            saveScreenshot = findall(figure, ...
                "Tag", "labkitAppUtilityScreenshot");
            testCase.verifyEqual(plotMenu.Parent, toolsMenu);
            testCase.verifyEqual(screenshotMenu.Parent, toolsMenu);
            testCase.verifyEqual(copyScreenshot.Parent, screenshotMenu);
            testCase.verifyEqual(saveScreenshot.Parent, screenshotMenu);
            testCase.verifyEqual(string(copyScreenshot.Text), ...
                "Copy to Clipboard");
            testCase.verifyEqual(string(saveScreenshot.Text), ...
                "Save to File...");
            testCase.verifyClass(copyScreenshot.MenuSelectedFcn, ...
                "function_handle");
        ...
        function updatesVersionedDocumentTitleAcrossSaveAndEdit(testCase)
            ...
            toolsMenu = findall(figure, ...
                "Tag", "labkitAppUtilityToolsMenu");
            projectMenu = findall(figure, ...
                "Tag", "labkitAppUtilityProjectStateMenu");
            saveState = findall(figure, ...
                "Tag", "labkitAppUtilitySaveState");
            loadState = findall(figure, ...
                "Tag", "labkitAppUtilityLoadState");
            testCase.verifyEqual(projectMenu.Parent, toolsMenu);
            testCase.verifyEqual(saveState.Parent, projectMenu);
            testCase.verifyEqual(loadState.Parent, projectMenu);
...
main = labkit.app.layout.tab("mainTab", "Main", { ...
    labkit.app.layout.section("visualInputs", "Inputs", { ...
        labkit.app.layout.fileList("visualFile", Label="Image", ...
            ...
            ChooseTooltip="Choose one calibrated probe image."), ...
    ...
    labkit.app.layout.section("visualActions", "Actions", { ...
        labkit.app.layout.group("visualButtons", { ...
            labkit.app.layout.button("visualRun", "Run", @noOp, ...
                Tooltip="Run the visual probe."), ...
            labkit.app.layout.button("visualReset", "Reset", @noOp, ...
                Tooltip="Reset the visual probe.")}, ...
        ...
        labkit.app.layout.group("visualAutoButtons", { ...
            labkit.app.layout.button("visualAutoFirst", "First", @noOp, ...
                Tooltip="Run the first visual action."), ...
            labkit.app.layout.button("visualAutoSecond", "Second", @noOp, ...
                Tooltip="Run the second visual action."), ...
            labkit.app.layout.button("visualAutoThird", ...
                "Third action", @noOp, ...
                Tooltip="Explain the third scientific action.")})}), ...
...
content = labkit.app.layout.group("controls", { ...
    labkit.app.layout.button("run", "Run", @runAnalysis, ...
        Tooltip="Run the probe analysis."), ...
    labkit.app.layout.button("resetView", "Reset view", @resetView, ...
        Tooltip="Accept the renderer's fitted plot limits."), ...
...
view = labkit.app.view.Snapshot() ...
    ...
    .renderPlot("preview", state.project.lineWidth, ...
        ViewRevision=state.session.plotViewRevision);
end
...
function state = resetView(state, ~)
state.session.plotViewRevision = state.session.plotViewRevision + 1;
end
