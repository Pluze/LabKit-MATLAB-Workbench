...
edited = labkit.ui.Command("tableEdited", @tableEdited, Role="tableEdit");
selected = labkit.ui.Command( ...
    "tableSelected", @tableSelected, Role="selection");
...
dataTable = labkit.ui.Layout.resultTable("data", ...
...
app = labkit.ui.Application(Command="labkit_ChronoProbe_app", ...
    ...
    Layout=labkit.ui.Layout.workbench({content, dataTable, ...
...
view = labkit.ui.Presentation() ...
    ...
    .table("data", state.session.tableData, ...
        Columns=["Group", "Value"], ColumnEditable=[true true]) ...
...
state.session.tableCells = selection.Cells;
end
