classdef LauncherGuiTest < matlab.uitest.TestCase
    ...
    methods (Test, TestTags = {'GUI', 'Structural'})
        ...
        function update_has_install_folder_safety_guards(testCase)
            root = setupLabKitTestPath();
            source = fileread(fullfile(root, "labkit_launcher.m"));
            body = launcherFunctionBlock(source, ...
            ...
            testCase.verifyFalse(isempty(strfind(body, 'assertNoUnmanagedInstallFiles(root);')), ...
                'Updater must refuse unmanaged files before overwriting LabKit files.');
            testCase.verifyFalse(isempty(strfind(source, ...
                'function assertNoUnmanagedInstallFiles(root)')), ...
                'Updater must define the unmanaged-file hygiene check.');
            testCase.verifyFalse(isempty(strfind(body, ...
            ...
            testCase.verifyTrue(isempty(strfind(body, 'createBackup(')), ...
                'Update flow should not make backup/rollback the normal UX.');
            testCase.verifyTrue(isempty(strfind(source, ...
                'User files that are not LabKit project files are left in place.')), ...
                'Update prompt must not imply unmanaged user files belong in the LabKit folder.');
        end

        function update_skips_unchanged_files_and_reports_copy_progress(testCase)
            root = setupLabKitTestPath();
            source = fileread(fullfile(root, "labkit_launcher.m"));
            updateBody = launcherFunctionBlock(source, ...
                'function result = launcherUpdateFromZipSource(root, source, progressFcn, preflightDone)', ...
                'function source = resolveStableZipSource()');
            ...
            testCase.verifyFalse(isempty(strfind(updateBody, ...
                'overlayManagedFiles(sourceRoot, root, newFiles, progressFcn)')), ...
                'Updater should pass the progress callback into managed-file copy.');
            testCase.verifyFalse(isempty(strfind(copyBody, 'notifyCopyProgress(')), ...
                'Managed-file copy should report progress while walking files.');
            testCase.verifyFalse(isempty(strfind(copyBody, 'filesAreEqual(source, target)')), ...
                'Managed-file copy should skip unchanged files instead of rewriting every file.');
            testCase.verifyFalse(isempty(strfind(source, ...
                'function tf = binaryFilesMatch(leftPath, rightPath)')), ...
                'Unchanged-file detection should compare file bytes, not zip timestamps.');
            testCase.verifyFalse(isempty(strfind(source, 'fopen(leftPath, "rb")')), ...
                'Binary comparison should open the source file in binary mode.');
            testCase.verifyFalse(isempty(strfind(source, 'fopen(rightPath, "rb")')), ...
                'Binary comparison should open the target file in binary mode.');
            testCase.verifyFalse(isempty(strfind(source, 'blockSize = 1024 * 1024')), ...
                'Binary comparison should read large files in bounded chunks.');
        end
        ...
        function version_manager_uses_update_safety_path(testCase)
            ...
            testCase.verifyFalse(isempty(strfind(source, ...
                ...
                'Launcher should expose one unmanaged-file policy helper for UI and update checks.');
            testCase.verifyFalse(isempty(strfind(source, ...
                'manifestFiles = readManifest(root);')), ...
                'Launcher hygiene should use the managed manifest when one exists.');
            testCase.verifyFalse(isempty(strfind(source, ...
                'labkit_launcher:MissingManagedManifest')), ...
                'Launcher should refuse legacy LabKit folders that lack a managed manifest.');
            testCase.verifyFalse(isempty(strfind(source, ...
                'isLauncherBootstrapPath')), ...
                'Launcher should only allow single-file bootstrap installs without a manifest.');
