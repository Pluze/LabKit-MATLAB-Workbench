...
function checkRequestErrors()
    ...
    assertThrows(@() labkit.ui.app.dispatchRequest('probe_app', {42}, 0, handlers), ...
        'probe_app:UnsupportedInput', 'Nonstrings should be unsupported app input.');
    assertThrows(@() labkit.ui.app.dispatchRequest('probe_app', {'__labkit_test__'}, 0, handlers), ...
        'probe_app:InvalidTestRequest', 'Test hooks require a command name.');
    assertThrows(@() labkit.ui.app.dispatchRequest('probe_app', {'__labkit_test__', 'missing'}, 0, handlers), ...
        'probe_app:UnknownTestCommand', 'Unknown test commands should fail with the canonical id.');
    assertThrows(@() labkit.ui.app.dispatchRequest('probe_app', {'__labkit_test__', 'echo'}, 0, handlers), ...
        'probe_app:InvalidTestArguments', 'Invalid test argument counts should fail with the canonical id.');
    assertThrows(@() labkit.ui.app.dispatchRequest('probe_app', {'__labkit_test__', 'echo', 'one'}, 2, handlers), ...
        'probe_app:TooManyOutputs', 'Too many requested outputs should fail with the canonical id.');
    assertThrows(@() labkit.ui.app.dispatchRequest('probe_app', {'debug'}, 3, handlers), ...
        'probe_app:TooManyOutputs', 'Too many debug outputs should fail with the canonical id.');
    assertThrows(@() labkit.ui.app.dispatchRequest('probe_app', {'debug', 42}, 0, handlers), ...
        'probe_app:InvalidTestRequest', 'Debug options should be a struct.');
    assertThrows(@() labkit.ui.app.dispatchRequest('probe_app', {'debug', struct(), struct()}, 0, handlers), ...
