...
function checkScaleBarPanelHelper(h)
    fig = uifigure('Visible', 'off', 'Name', 'labkit_scale_bar_panel_probe');
    cleaner = onCleanup(@() delete(fig)); %#ok<NASGU>
    grid = uigridlayout(fig, [2 1]);

    calls = struct('measure', 0, 'calibration', 0, 'bar', 0, 'place', 0);
    ui = labkit.ui.createScaleBarPanel(grid, 2, ...
        struct('onMeasureReference', @onMeasure, ...
        'onCalibrationChanged', @onCalibration, ...
        'onScaleBarChanged', @onBar, ...
        'onPlaceScaleBar', @onPlace));
    ...
    assert(h.sameStringCell(ui.controls.unitDropdown.Items, {'um', 'mm', 'nm', 'cm'}), ...
    ...
    assert(pxPerUnit == 4 && strcmp(unitName, 'um'), ...
    ...
    whiteSpec = ui.scaleBarSpec([600 1000 3]);
    ...
    assert(calls.calibration == 1 && calls.measure == 1 && calls.place == 1, ...
        'Scale-bar panel should wire calibration, measure, and place callbacks.');
    ...
    ui.setEnabled(struct('hasImage', true, 'referenceEditActive', true, ...
