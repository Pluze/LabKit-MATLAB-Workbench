...
function checkECGPrint()
    ...
    assertComponentCounts(fig, struct('Button', 6, 'DropDown', 5, ...
    ...
    assertDropdownGroups(fig, [ ...
        ...
        dropdownGroup({'QRS streaming', 'Pan-Tompkins', 'Local peaks'}, 1), ...
...
function checkStandardWorkbenchShellHelper()
    ...
    defaultUi = labkit.ui.createStandardWorkbenchShell( ...
        'labkit_standard_workbench_shell_default_probe', ...
        [40 30 1200 760], ...
        340);
    cleaner2 = onCleanup(@() delete(defaultUi.fig)); %#ok<NASGU>
    assert(strcmp(defaultUi.rightPanel.Title, 'Plots'), ...
        'Standard workbench compatibility wrapper should default the right panel title.');
    assert(isequal(defaultUi.rightGrid.RowHeight, {'1x'}), ...
        'Standard workbench compatibility wrapper should default right-grid row heights.');
end
...
function checkTabbedDualPlotShellHelper()
    ...
    partial = labkit.ui.createTabbedDualPlotShell( ...
        'labkit_tabbed_dual_plot_shell_partial_probe', ...
        [40 30 1200 760], ...
        360, ...
        struct('plotsPanel', 'Custom Plots'));
    cleaner2 = onCleanup(@() delete(partial.fig)); %#ok<NASGU>
    assert(strcmp(partial.rightPanel.Title, 'Custom Plots'), ...
        'Tabbed dual-plot compatibility wrapper should accept partial label overrides.');
    assert(strcmp(partial.leftPanel.Title, 'Controls'), ...
        'Tabbed dual-plot compatibility wrapper should default missing labels.');
end
