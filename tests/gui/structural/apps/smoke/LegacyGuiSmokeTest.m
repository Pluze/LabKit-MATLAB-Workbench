classdef LegacyGuiSmokeTest < matlab.uitest.TestCase
    %LEGACYGUISMOKETEST Official wrapper for migrated legacy coverage.

    methods (Test, TestTags = {'GUI', 'Structural', 'Smoke'})
        function test_gui_smoke(testCase)
            setupLabKitTestPath();
            legacy_test_gui_smoke();
        end
    end
end

function legacy_test_gui_smoke()
