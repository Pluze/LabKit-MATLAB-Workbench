...
- Debug synthetic sample packs are implemented as app-owned runtime generators.
  They write clean-room representative, malformed, and boundary files under
  each debug session's `samples/` folder, record `manifest.json`, and leave
  app startup state empty so debug mode can still be used with user-selected
  real data.
- Debug artifact paths are documented in `docs/ui.md` and `docs/testing.md`;
  reopen this ledger only if scans expose new migration debt.
