...
- Package-root app `run.m` orchestration and `+ui/runApp.m` lifecycle adapters
  are retired.
- All twenty public apps now use Runtime V2: `chrono_overlay`, `cic`, `csc`, `eis`,
...
phase: 5/full-app-migration-waves
status: complete
completed contracts: CIC source-only project, durable analysis/view parameters, session-owned decoded and analyzed DTA cache, native presenter and registered axis renderer, standard result manifest; CSC source-only project, bound comparison/plot settings, session-owned curve selection and decoded DTA cache, pure comparison/table/two-axis presentation, and result manifests for both export forms; EIS source-only project, bound plot/export parameters, session-owned multi-file selection and decoded cache, pure summary/overlay presentation, and standard result manifest; VT Resistance source-only project, bound resistance/plot settings, session-owned decoded and analyzed DTA cache, pure summary/table/two-axis presentation, and standard result manifest; app-neutral controlled interval drag and semantic wheel events; RHS Preview portable RHS/protocol/filter sources, durable channel/filter decisions, session-owned header/index/window caches, pure presentation, controlled waveform interval/zoom, and manifests for both JSON outputs; Nerve Response Analysis portable filter/protocol sources, durable analysis limits/export record, session-owned parsed JSON/analysis cache, bound parameters, pure presentation and registered count/issue renderer, and standard JSON manifest; Response Review Stats portable analysis/segment source, durable metric windows/export record, one cohesive cache rebuild path, session-owned metrics/summary/aligned data, pure presentation and registered preview, and standard CSV manifest; ECG Print portable recording source, durable import/channel/ROI/filter/segment/view parameters and compact analysis/export results, session-owned decoded signal/event/segment/template/measurement caches, pure four-axis presentation, model-based PNG export, and manifests for both exports; Gait Analysis portable pose source, durable role/calibration/detection parameters and analysis/export results, session-owned decoded pose/fingerprint/output convenience, bound options, pure trajectory/angle/step renderer, and four-output manifest; Curvature portable image source, durable curve/calibration/fit/length/export state, session-owned decoded image/edit mode/preview geometry, controlled curve/reference editors, pure presenter/renderer, model-based PNG export, both result manifests, and app-neutral anchor-path projection; Image Enhance portable image sources, durable shared/per-image steps and ROI annotations, selected-image-only decoded/preview cache, pure presenter/renderer, controlled white-ROI rectangle, lazy batch loading at export, preserved CSV manifest, and standard result manifest; Image Match portable reference/source records, durable match parameters/history/export results, selected-source/reference preview caches, pure presenter/renderer, lazy batch loading at export, preserved CSV manifest, standard result manifest, and app-neutral stable source-list reconciliation reused by Image Enhance; FLIR Thermal portable radiometric source records, durable display/range/temperature-reading annotations and export results, selected-item-only decoded cache, pure two-axis presentation, preserved CSV/image outputs plus standard result manifest, and app-neutral transient region selection for point-or-drag readings; Focus Stack portable image sources, bound fusion/registration settings, compact durable run/export results, session-owned decoded/aligned/full-result matrices, pure paired-image presentation, standard result manifests, and an app-neutral input-folder dialog service; Figure Studio portable FIG sources or embedded plot data, durable style/export settings, session-owned decoded plot model, pure presenter and registered renderer, managed preview-resize resources, model-based exports, and standard result manifests
migrated apps: cic; csc; eis; vt_resistance; rhs_preview; nerve_response_analysis; response_review_stats; ecg_print; gait_analysis; curvature; image_enhance; image_match; flir_thermal; focus_stack; figure_studio
compatibility retained: CIC calculations, threshold defaults, pulse modes, summary meanings, plot annotations, 8-column batch table, and 14-column CSV contract; CSC CV/CT parsing, all/single-cycle selection, comparison modes, edge-cycle filtering, plot/trim behavior, six-column visible table, full all-cycle CSV, and point-level CV CSV contracts; EIS axis choices, multi-selection, Nyquist/Bode plotting, log-axis behavior, summary, and plot-data CSV contract; VT Resistance pulse detection, steady-window and voltage modes, resistance calculations, double-axis annotations, 9-column batch table, and 15-column CSV contract; RHS lazy index/window reads, adaptive pan/zoom, waveform ROI, protocol draft, manual filter curation, and existing JSON schemas/filenames; Nerve Response Analysis filter/protocol semantics, recording limits, analysis calculations, counts/issues preview modes, summary/details, default output folder, and JSON filename/schema; Response Review Stats JSON/CSV input semantics, metric windows, automatic reload, Summary/Aligned preview, summary/details, default output folder, and CSV filename/schema; ECG import overrides, channel/ROI/filter/peak/segment/template settings, four plot meanings, summary rows, segment CSV columns, waveform PNG, and filenames; gait pose parsing, role/calibration/detection options, duplicate-run semantics, trajectory/angle/step previews, summary/step tables, four CSV filenames/schemas, and output-folder workflow; Curvature anchor editing, Catmull-Rom display path, reference calibration, circle fit/length calculations, scale-bar placement, result table, CSV, overlay PNG, and filenames; Image Enhance tool formulas/defaults, batch/per-image histories, white-ROI semantics, preview modes, export formats, enhanced filenames, and CSV manifest; Image Match methods/strengths, shared ordered history, preview modes, output formats, matched filenames, and CSV manifest; Focus Stack image loading/folder discovery, presets, optional middle-image registration, Laplacian fusion, duplicate-run detection, paired previews, summary table, and three export filenames/schemas
tests: CIC export/project/presenter, view, and calculation tests plus hidden GUI load/recompute/plot/export/project-reopen workflow passed 9/9; CSC 42-test electrochem non-GUI selection and hidden GUI load/compare/plot/export/project-reopen workflow passed at the migration checkpoint; EIS hidden GUI load/selection/log-plot/export/project-reopen workflow passed at its focused checkpoint; VT Resistance passed 42/42 electrochem non-GUI tests and its hidden Runtime V2 load/recompute/plot/export/project-reopen workflow; Runtime V2 interaction-hub tests passed 4/4 including interval wheel routing and transient region selection; RHS Preview passed 33/33 neurophysiology non-GUI tests and its hidden indexing/preview/filter/export/project-reopen workflow; Nerve Response Analysis hidden analyze/export/manifest/project-reopen/cache-rebuild workflow passed at its focused checkpoint; Response Review Stats hidden auto-load/preview/export/manifest/project-reopen/cache-rebuild workflow passed at its focused checkpoint; ECG Print passed 27/27 wearable non-GUI tests and its hidden parse/analyze/four-axis/export/manifest/project-reopen/cache-rebuild workflow; Gait Analysis passed 21/21 gait non-GUI tests and its hidden load/analyze/preview/four-CSV/manifest/project-reopen/cache-rebuild workflow; Curvature passed 22/22 image-measurement non-GUI tests, 15/15 UI/package contract tests, and its hidden calibrate/trace/fit/length/two-export/two-manifest/project-reopen/cache-rebuild workflow; Image Enhance passed 39/39 image-family non-GUI tests, its hidden per-image history/export/two-manifest/project-reopen/cache-rebuild workflow, and 6/6 representative UI/app GUI workflows through `buildtool changedFast`; Image Match passed the same 39/39 image-family tests, 7/7 UI unit tests, 6/6 representative UI/app GUI workflows, and its hidden match/export/two-manifest/project-reopen/cache-rebuild workflow; FLIR Thermal passed 5/5 focused unit tests and its two hidden load/range/export/result-manifest/project-reopen/cache-rebuild workflows; Focus Stack passed 22/22 image-family unit tests plus its hidden source/preset/run/export/manifest/project-reopen/cache-rebuild/input-folder workflow and 30/30 UI framework tests; Figure Studio passed 4/4 source/export unit tests and its four hidden load/style/resize/axes-handoff/quick-export/package-export/manifest/project-reopen workflows
next phase: migrate the nested private app, then reduce the superseded UI surface
blocker:
```
...
| `curvature` | 1971 -> 2004 | 609 -> 385 | accept near-neutral total after explicit project/session/presenter/renderer/export contracts; removed closure state, Startup, UI-handle mapping, runtime appdata writes, manual refresh bridges, and UI-axis export while adding one narrow app-neutral anchor-path API shared with managed editors |
| `image_enhance` | 1858 -> 2057 | 637 -> 482 | accept explicit source/project/session/presenter/result ownership for a multi-image editor; removed closure state, 17-function manual refresh chain, UI control reads/writes, ROI handles/listeners, eager batch decoding, and legacy dual-state test paths without adding a public framework API |
| `image_match` | 1870 -> 1890 | 607 -> 386 | naturally near-neutral after framework-owned source reconciliation; removed closure state, direct control reads/writes, manual refresh chain, and eager batch decoding while retaining the app-owned matching algorithm and explicit project/session/presenter/result contracts |
| `flir_thermal` | 2440 -> 2659 | 634 -> 608 | accept explicit source/project/session/presenter/result contracts and keep thermal conversion/export logic app-owned; removed closure state, direct controls, manual refresh/state mirroring, decoded-matrix persistence, and the app-owned point/ROI drag tool while adding one app-neutral transient region-selection interaction |
| `focus_stack` | 1690 -> 1841 | 314 -> 310 | accept explicit project/session/presenter/result contracts with a stable largest file; removed direct control reads/writes, eager durable matrices, manual render/control refresh, startup dispatch, dialog/debug glue, and added only one app-neutral input-folder service |
| `figure_studio` | 2171 -> 2375 | 598 -> 462 | accept explicit portable plot/session/resource/result contracts while reducing the largest workflow file; removed axes handles from state, direct UI-registry access, manual refresh, preview-driven export, and app-owned lifecycle cleanup, adding only semantic preview-axis lookup for managed startup resources |
