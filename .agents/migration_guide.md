...
- There are 17 supported app packages. Thirteen currently launch through
  `labkit.ui.app.run(<slug>.definition(), request)` using transitional
  `+state`, `+actions`, `+ui`, and `+view` adapters. Four still launch
...
Crop, CSC, DIC Preprocess, and Image Match transitional migrations are
committed or in progress on the active branch. This is not a final state. The
route remains open until every app launches through a framework-owned
definition and package-root eager orchestration is removed.
...
3. Migrate the four remaining package-root runners to transitional
   definitions.
...
2. App migration
   - Finish retiring the four remaining package-root runners.
