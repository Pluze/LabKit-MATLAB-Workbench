...
- Package-root app `run.m` orchestration and `+ui/runApp.m` lifecycle adapters
  are retired.
- Eleven public apps now use Runtime V2: `chrono_overlay`, `cic`, `csc`, `eis`,
  `vt_resistance`, `dic_postprocess`, `batch_crop`, `dic_preprocess`, and
  `video_marker`, plus `rhs_preview` and `nerve_response_analysis`. Nine public apps and
...
next app: response_review_stats
blocker:
```
