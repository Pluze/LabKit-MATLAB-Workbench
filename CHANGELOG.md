...
- DIC postprocess can export a standalone strain colorbar PNG plus a CSV mapping strain levels to RGB values, and DIC preprocess can draw editable spline ROI boundaries with add/move/delete/preview controls before saving white-inside/black-outside ROI masks.
...
- DIC preprocess ROI editing now handles preview scroll-wheel zoom directly during add/move/delete modes and no longer needs a separate pan/zoom mode button.
