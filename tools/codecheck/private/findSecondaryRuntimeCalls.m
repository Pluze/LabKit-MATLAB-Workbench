...
% nearby marker. The NI-DMM adapter has exact file/count allowances plus one
% file marker. All other categories are zero-tolerance.
    ...
    for file = string(files(:)).'
        ...
        for ruleIndex = 1:numel(rules)
            ...
            for matchIndex = 1:numel(starts)
                ...
                if isAllowedSecondaryBoundary(relativeFile, ...
...
function tf = isAllowedSecondaryBoundary(file, category, source, startIndex, ...
        allowances, allowanceCounts)
