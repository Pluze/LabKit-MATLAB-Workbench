function htmlFile = writeCodecheckReport(jsonFile, htmlFile)
%WRITECODECHECKREPORT Write a self-contained HTML view for codeIssues JSON.
%
% Expected caller: tools/codecheck/runCodecheckReport.m. Inputs are the native
% MATLAB codeIssues JSON export and output HTML path. Output is the written
% HTML file path. Side effects are limited to writing that HTML file.

    jsonText = string(fileread(jsonFile));
    payload = jsondecode(char(jsonText));
    summary = summarizePayload(payload);
    sources = collectSources(payload);
    html = buildHtml(jsonText, summary, sources);
    writeUtf8(htmlFile, html);
end

function summary = summarizePayload(payload)
...
function sources = collectSources(payload)
    paths = strings(0, 1);
...
function html = buildHtml(jsonText, summary, sources)
    ...
    parts = [
        ...
        "<script id=""summary-json"" type=""application/json"">" + summaryJson + "</script><script id=""source-json"" type=""application/json"">" + sourcesJson + "</script><script id=""codeissues-json"" type=""application/json"">" + jsonText + "</script><script>" + clientScript() + "</script></body></html>"
        ];
...
function script = clientScript()
    lines = [
        "const payload=JSON.parse(document.getElementById('codeissues-json').textContent);const summary=JSON.parse(document.getElementById('summary-json').textContent);const sourceRows=asArray(JSON.parse(document.getElementById('source-json').textContent));const sourceMap=new Map(sourceRows.map(s=>[text(s.path),asArray(s.lines).map(text)]));const rawIssues=asArray(payload.Issues).map((x,i)=>normIssue(x,i,false));const rawSuppressed=asArray(payload.SuppressedIssues).map((x,i)=>normIssue(x,i,true));let sortKey='severity',sortDir=1,selected=null;"
        "function asArray(v){if(!v)return[];return Array.isArray(v)?v:[v]}function text(v){return v===undefined||v===null?'':String(v)}function basename(p){p=text(p).replace(/\\/g,'/');return p.split('/').filter(Boolean).pop()||p}function normIssue(x,i,suppressed){return{row:i+1,suppressed,severity:text(x.Severity).toLowerCase(),checkid:text(x.CheckID),file:basename(x.FullFilename||x.Location),path:text(x.FullFilename||x.Location),line:Number(x.LineStart||0),endLine:Number(x.LineEnd||x.LineStart||0),column:Number(x.ColumnStart||0),description:text(x.Description),fixability:text(x.Fixability)}}function sevRank(s){return s==='error'?0:s==='warning'?1:s==='info'?2:3}"
        "function setText(id,v){document.getElementById(id).textContent=v}function init(){setText('generatedAt',summary.date||payload.Date||'');setText('matlabRelease',summary.release||payload.Release||'');setText('errorCount',summary.errorCount||0);setText('warningCount',summary.warningCount||0);setText('infoCount',summary.infoCount||0);setText('issueCount',summary.issueCount||0);setText('suppressedCount',summary.suppressedIssueCount||0);setText('fileCount',summary.fileCount||asArray(payload.Files).length);populateCheckIds();bind();renderAll();selectInitialIssue()}"
        "function populateCheckIds(){const ids=[...new Set(rawIssues.concat(rawSuppressed).map(x=>x.checkid).filter(Boolean))].sort();const sel=document.getElementById('checkid');ids.forEach(id=>{const o=document.createElement('option');o.value=id;o.textContent=id;sel.appendChild(o)})}function bind(){['search','issueSet','severity','checkid','limit'].forEach(id=>document.getElementById(id).addEventListener('input',renderAll));document.getElementById('reset').onclick=()=>{search.value='';issueSet.value='active';severity.value='all';checkid.value='all';limit.value=500;renderAll()};document.querySelectorAll('th[data-sort]').forEach(th=>th.onclick=()=>{const k=th.dataset.sort;if(sortKey===k)sortDir*=-1;else{sortKey=k;sortDir=1}renderTable(filtered())})}"
        "function filtered(){const q=search.value.trim().toLowerCase();const sev=severity.value;const cid=checkid.value;let rows=issueSet.value==='suppressed'?rawSuppressed.slice():issueSet.value==='all'?rawIssues.concat(rawSuppressed):rawIssues.slice();return rows.filter(r=>(sev==='all'||r.severity===sev)&&(cid==='all'||r.checkid===cid)&&(!q||[r.suppressed?'suppressed':'active',r.severity,r.checkid,r.file,r.path,r.description,r.fixability].join(' ').toLowerCase().includes(q)))}function cmp(a,b){let av=sortKey==='set'?(a.suppressed?1:0):a[sortKey],bv=sortKey==='set'?(b.suppressed?1:0):b[sortKey];if(sortKey==='severity'){av=sevRank(a.severity);bv=sevRank(b.severity)}if(sortKey==='line'||sortKey==='row'||sortKey==='set'){av=Number(av);bv=Number(bv)}return (av>bv?1:av<bv?-1:0)*sortDir}function sortedRows(rows){return rows.slice().sort(cmp)}"
        "function renderAll(){const rows=filtered();renderTable(rows);renderFiles(rows);renderChecks(rows)}function renderTable(rows){rows=sortedRows(rows);const max=Math.max(1,Number(limit.value)||500);const tb=document.querySelector('#issuesTable tbody');tb.innerHTML='';rows.slice(0,max).forEach(r=>{const tr=document.createElement('tr');if(selected&&selected.path===r.path&&selected.line===r.line&&selected.checkid===r.checkid&&selected.suppressed===r.suppressed)tr.className='selected';tr.onclick=()=>selectIssue(r);tr.innerHTML='<td class=num>'+r.row+'</td><td><span class=""badge '+(r.suppressed?'suppressed':'')+'"">'+(r.suppressed?'suppressed':'active')+'</span></td><td><span class=""badge sev-'+escAttr(r.severity)+'"">'+esc(r.severity||'unknown')+'</span></td><td>'+esc(r.checkid)+'</td><td class=issueFile title=""'+escAttr(r.path)+'"">'+esc(r.file)+'</td><td class=num>'+esc(r.line||'')+'</td><td class=desc>'+esc(r.description)+'</td>';tb.appendChild(tr)});if(!tb.children.length){tb.innerHTML='<tr><td colspan=7 class=empty>No matching issues.</td></tr>'}}function selectIssue(r){selected=r;renderDetail(r);renderSource(r);renderTable(filtered())}function selectInitialIssue(){const rows=sortedRows(filtered());if(rows.length)selectIssue(rows[0]);else sourceView.innerHTML='<div class=empty>No source selected.</div>'}"
