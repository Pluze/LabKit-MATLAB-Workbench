...
The default suite is listed in `tests/run_all_tests.m` under `defaultTests()` and covers startup/root-entry boundaries, parsers, utilities, data accessors, DTA facade discovery/detection/loading/session helpers, template source boundaries, pulse detection, analysis functions, plotting helpers, export table builders, session helpers, UI-table helpers, and app-entry resolution.
