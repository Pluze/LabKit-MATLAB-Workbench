...
Later cleanup removed the root-level original command wrappers, added package-backed app entry points for chrono overlay/export and CIC, and removed the old `legacy/` GUI directory. Subsequent boundary cleanup moved DTA item/session construction, pulse detection, parser/session IO, and parsed table/curve access behind `gamrywb.dta.*`. Public `+data` and `+io` packages were removed so the app-facing library surface is now `gamrywb.ui.*` plus the current `gamrywb.dta.*` data-family API. The old command names are now reference history rather than runtime entry points.
