...
apps/<family>/<app_slug>/appDefinition.m
apps/<family>/<app_slug>/buildUiLayout.m
apps/<family>/<app_slug>/initialAppState.m
...
not add a folder with the same stem as an existing file, such as both
`actions.m` and `actions/`.
...
Authors should think in terms of an app definition, UI layout, initial state,
specific user actions, specific rendered outputs, and optional
`ops/io/export/debug` roles. The `+<app_slug>` adapter exists to connect that
source to MATLAB package resolution and `labkit.ui.app.run`.
...
`appDefinition.m` and the generated `definition.m` together declare the app's
runtime contract. The generated adapter returns a plain struct created with
...
For nontrivial apps, `buildUiLayout.m` should make the page hierarchy obvious
at the top of the file. Keep the app constructor shallow, then use local builder
...
Keep one visible structure per role. Do not keep both singular and directory
forms for the same concept (`actions.m` plus `actions/`, `view.m` plus
`views/`, or `model.m` plus `model/`). When a role grows, either keep cohesive
local functions or add specific sibling files such as
`handleExportRequested.m` and `renderPreviewAxes.m`. The runtime adapter may still use
MATLAB package folders such as `+actions` and `+view`; those are framework
adapter details, not a second author-facing structure.
