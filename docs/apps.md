...
DIC preprocess image registration, inline right-preview ROI selection, paired-crop geometry, and point-by-point closed ROI mask drawing are local details of `apps/dic/labkit_DICPreprocess_app.m`. After applying a crop, the right preview shows the reference crop on top and the moving/aligned crop on the bottom. ROI masks are saved as white-inside / black-outside binary PNG images.
