...
ECG print and SNR exploration lives in `apps/wearable/labkit_ECGPrint_app.m`. The app loads MAT timetable or delimited table recordings through `labkit.biosignal.readRecording`, previews text-file headers for ambiguous CSV imports, lets the user explicitly refresh parsing after changing header/time/signal-column options, chooses a numeric channel and time ROI, runs generic filtering/peak detection/segmentation/template/SNR helpers, and plots waveform peaks, noise RMS over time, SNR over time, and template residual views. It exports per-segment SNR CSV files plus a waveform PNG. Multi-file or multi-class SNR statistics are intentionally left for a separate wearable statistics app rather than being mixed into the single-signal viewer.
