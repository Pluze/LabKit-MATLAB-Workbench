...
| `apps/electrochem/csc/+csc/+ui/runApp.m` | electrochem | App-owned `+ops` exists, but runner still owns display formatting, dropdown defaults, and trim plotting. | Move display/selection helpers into `+view` before changing load callbacks. |
...
| Curve dropdown population and default X/Y selection | runner (`updateDropdowns`, `autoSetDefaults`, `setDropdownValueIfExists`) | `csc.view` |
| Charge/CSC display formatting | runner local `formatChargeAndCSC` | `csc.view` |
...
Move `formatChargeAndCSC` and dropdown default selection into `csc.view` with
direct unit tests. These are deterministic and do not need UI state when passed
plain values.
...
Add an electrochem unit test for charge/CSC display formatting and dropdown
selection fallback behavior. Use simple structs or values; avoid launching the
CSC app.
