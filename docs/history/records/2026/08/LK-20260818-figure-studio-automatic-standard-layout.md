...
```labkit-change
id: LK-20260818-figure-studio-automatic-standard-layout
date: 2026-08-18
sequence: 182
type: fix
...
scope: Figure Studio automatic import layout
scope: Plot handoff canvas geometry
```
...
Focused product evidence covers axes handoff initialization, source-capability evidence covers separation of source geometry from the reference style, and the bounded hidden-GUI workflow verifies FIG preview and export. Manual review compares a T-Test Wizard handoff with the maintained reference proportions.

## Evidence

The reported T-Test handoff uses three long category labels that exceed one horizontal category slot at the calibrated type size. The repaired contract retains the reference frame and measured style values, then balances each long label across two horizontal lines.
