...
Focused App SDK evidence covers delayed action feedback, leaf-only input freezing, reentrant action rejection, the absence of visible busy state for sliders and managed rectangles, durable journal stages, previous-active export, compact diagnostic state, source/resource contracts, and removal of the generic Project State and synthetic-input menus. Definition and initial-session conformance cover every built-in App on the opaque state boundary. Batch Crop result and presentation evidence covers current-manifest round trips, saved-row restoration, changed-source rejection, stable columns, and the restore action declaration. Video Marker evidence covers explicit archive round trips, continued rendering, and the absence of implicit MAT writes. Gait source and workbench evidence covers current Video Marker archive parsing, source-derived scientific options, analysis, presentation, and export. Behavior specs directly cover the remaining generated inputs; fixture-only specifications and manual-replay builders are removed. Biosignal, test-catalog, architecture, code-analysis, Skill validation, and documentation checks cover the retired consumers and public surface. Native pointer feel and renderer-specific paint timing remain manual GUI boundaries.
