...
- `+ui`: VT resistance and CIC batch table display data; shared app axes creation/reset, log, multi-select and single-select file-listbox refresh, multi-file and single-select file-panel, info/log text-area, plot-options panel, simple labeled-control, two-pane shell, tabbed dual-plot shell, and top/bottom plot-control helpers.
