...
```text
apps/ public app entry points; EIS is the first single-file app implementation
    ↓
...
- `apps/`: user-facing app entry points and app-specific implementations. EIS is currently a single public app source file. Chrono overlay, CSC, VT resistance, and CIC still use transitional `apps/private` launch bodies. EIS, Chrono overlay, CSC, VT, and CIC-specific analysis/export/plot helpers currently live under `apps/+gamrywb_apps` as app-side transitional code so numerical tests remain direct.
