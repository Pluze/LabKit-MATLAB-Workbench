...
Scale-bar responsibility is split deliberately:

- `labkit.ui.createScaleBarTool` owns the scale-bar panel controls, legal units, default control values, unit normalization, typed-reference parsing, two-endpoint reference editing through `createAnchorCurveEditor`, pixels-per-unit readout, placed scale-bar state, black/white overlay drawing, and user interaction mechanics inside the image axes.
- The app owns image acquisition, image redraw timing, passing the current image size and background image handle to the tool, deciding when scale editing conflicts with other edit modes, app logs and alerts, and domain calculations that consume `tool.calibration()`.

If scale-bar endpoint editing feels slow, misses drag responses, or conflicts with curve editing on the same axes, debug `createScaleBarTool` and `createAnchorCurveEditor` first. App-side issues are limited to integration mistakes such as not refreshing the current background image handle, leaving another editor active, or redrawing the axes at the wrong time.
