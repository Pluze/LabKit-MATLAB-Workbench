...
Public-repository `.m` source otherwise remains inside the MATLAB language runtime. It does not call Java, Python, Conda, .NET, shell commands, MEX/native libraries, or ActiveX/COM. One explicitly approved exception lets the package-private `labkit.nidmm` adapter call the end-user-installed official NI-DMM .NET runtime. That facade checks availability without opening hardware, revalidates when connecting, exposes only MATLAB-native data and opaque LabKit tokens, and never downloads, installs, bundles, or redistributes NI software. NI-DMM absence therefore disables that hardware workflow without preventing clean LabKit installation, documentation, tests, or other Apps. The repository guard scans production, tools, and tests; exact marked and counted allowances prevent another entry point from inheriting either the NI-DMM exception or the five test-infrastructure shell boundaries.
