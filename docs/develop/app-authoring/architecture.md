...
LabKit Apps run from Base MATLAB, their own App package, and public LabKit facades declared in their Definition requirements. App source never calls optional MathWorks Toolboxes, vendor SDKs, or secondary runtimes directly. Facades, launchers, and shipped maintainer tools otherwise run from Base MATLAB and repository-owned code. Installed local analysis workflows do not require first-use dependency downloads. Launcher installation and version downloads use the network, and device or network-source workflows retain their documented connection requirements. This keeps source checkouts, offline packages, and restored lab systems reproducible.

Public-repository `.m` source otherwise remains inside the MATLAB language runtime. It does not call Java, Python, Conda, .NET, shell commands, MEX/native libraries, or ActiveX/COM. One explicitly approved exception lets three package-private `labkit.nidmm` adapters call the end-user-installed official NI-DMM and NI System Configuration .NET runtimes. That facade checks availability, discovers local DMM aliases without opening hardware, revalidates when connecting, exposes only MATLAB-native data and opaque LabKit tokens, and never downloads, installs, bundles, or redistributes NI software. NI software absence therefore disables that hardware workflow without preventing clean LabKit installation, documentation, tests, or other Apps. The repository guard scans production, tools, and tests; exact marked and counted allowances prevent another entry point from inheriting either the NI-DMM exception or the five test-infrastructure shell boundaries.

When Base MATLAB cannot satisfy a proposed contract, that proposal stops at an architecture decision. A hardware dependency is accepted only through a separately approved focused facade and exact private adapter, not through an App fallback branch or generic dependency mechanism. A clean CI runtime without optional Toolboxes or vendor drivers exercises every driver-independent contract plus the documented unavailable-driver outcome, and focused source guards prevent concrete external entry points from spreading.
...
| `labkit.nidmm` | GUI-free NI-DMM runtime diagnostics, connection, voltage/current/resistance/diode configuration, and hardware-buffered sampling. |
