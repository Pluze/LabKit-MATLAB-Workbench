...
labkit/biosignal           split biosignal import, channel extraction, processing, ECG peaks, segments, SNR, and group comparison
...
Shared setup, structural GUI assertions, and focused support routines live under `tests/helpers/`. Keep helpers limited to setup and assertions; app-specific layout contracts, formulas, result schemas, export formats, and expected scientific values should remain in focused suite tests. `guiTestHelpers` provides shared lookup/assert/invoke/cleanup helpers only; app and reusable-UI layout expectations live in the owning `test_gui_layout_*.m` files. The custom runner reports per-test and per-suite durations so slow checks can be identified without running a profiler.

Architecture guardrails are split by concern under `tests/suites/project/`: public package surface, reusable package dependency boundaries, app entrypoint boundaries, and app-owned workflow boundaries. Add new guardrails to the narrowest matching file rather than rebuilding a broad aggregate test.
...
- app test hooks use `appName("__labkit_test__", "commandName", ...)`, and debug hooks use `[fig, debug] = appName("__labkit_debug__", opts)`
...
- keep architecture guardrails in the focused `tests/suites/project/test_*_boundaries.m` or `test_package_public_surface.m` files rather than duplicating them in numerical analysis tests
