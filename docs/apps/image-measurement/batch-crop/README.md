...
3. Click blank preview space to place the crop center. Drag the center marker
   or anywhere inside the highlighted ROI to move it, or enter center X/Y.
...
interior; click outside it to set its center. Change crop width and height with
their controls. Editing the ROI updates the selected task immediately;
switching tasks restores that task's geometry. ROI refreshes preserve axes
zoom.
