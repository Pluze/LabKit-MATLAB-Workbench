...
Both spatial plots use the current pixels-per-unit calibration and unit name for their coordinates and axis labels while retaining the image origin and downward-positive Y direction. They preserve equal X/Y data units so gait shape is not stretched by the available panel geometry. Loading a different Video Marker source, running analysis, changing scale options, or changing the selected step fits the new data once; status and presentation redraws preserve the user's zoom.
