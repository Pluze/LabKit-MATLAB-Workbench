...
function drawExtremaLabel(ax, x, y, labelText, color, side, yOffsetFraction)
    ...
    limitsX = xlim(ax); limitsY = ylim(ax);
    xyText = [x + xOffset * diff(limitsX), y + yOffsetFraction * diff(limitsY)];
    xyText(1) = min(max(xyText(1), limitsX(1) + .05 * diff(limitsX)), limitsX(2) - .05 * diff(limitsX));
    xyText(2) = min(max(xyText(2), limitsY(1) + .05 * diff(limitsY)), limitsY(2) - .05 * diff(limitsY));
