% Expected caller: App SDK runtime SyntheticInputs lifecycle and unit tests. Input is a
...
%WRITESAMPLEPACK Write CIC debug chrono DTA files.
    ...
    project.inputs.sources = sampleContext.sourceRecord( ...
        "dta1", "chrono", currentPath);
    pack = labkit.app.synthetic.Pack( ...
        Scenario="representative-chrono", InitialInput=project, ...
        Artifacts={ ...
...
function writeTextFile(filepath, text)
    ...
    if fid < 0
        error("cic:syntheticInputs:SampleWriteFailed", "Could not write synthetic input file: %s.", filepath);
    end
