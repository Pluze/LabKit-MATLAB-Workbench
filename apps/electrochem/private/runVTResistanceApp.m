...
function fig = runVTResistanceApp(debugLog)
%RUNVTRESISTANCEAPP Build and run the app body.
    ...
    function refreshResultsSummary()
        ...
        S.txtCathBaseWin.Value = vtResistanceWorkflow("formatDurationUs", A.cathBaselineWindow_s);
        S.txtAnodBaseWin.Value = vtResistanceWorkflow("formatDurationUs", A.anodBaselineWindow_s);
    ...
    function plotOneAxis(ax, A, xChoice, yChoice, showGrid)
        if strcmp(xChoice,'Sample #')
            x = A.pt;
            xlab = 'Sample #';
            cathStartX = vtResistanceWorkflow("interp1Safe", A.t, A.pt, A.pulse.cath_start);
            cathEndX = vtResistanceWorkflow("interp1Safe", A.t, A.pt, A.pulse.cath_end);
            anodStartX = vtResistanceWorkflow("interp1Safe", A.t, A.pt, A.pulse.anod_start);
            anodEndX = vtResistanceWorkflow("interp1Safe", A.t, A.pt, A.pulse.anod_end);
            cathBaseStartX = vtResistanceWorkflow("interp1Safe", A.t, A.pt, A.pulse.pre_start);
            cathBaseEndX = vtResistanceWorkflow("interp1Safe", A.t, A.pt, A.pulse.pre_end);
            anodBaseStartX = vtResistanceWorkflow("interp1Safe", A.t, A.pt, A.anodBaselineStart);
            anodBaseEndX = vtResistanceWorkflow("interp1Safe", A.t, A.pt, A.anodBaselineEnd);
            cSteadyStartX = vtResistanceWorkflow("interp1Safe", A.t, A.pt, A.cathSteadyStart);
            cSteadyEndX = vtResistanceWorkflow("interp1Safe", A.t, A.pt, A.cathSteadyEnd);
            aSteadyStartX = vtResistanceWorkflow("interp1Safe", A.t, A.pt, A.anodSteadyStart);
            aSteadyEndX = vtResistanceWorkflow("interp1Safe", A.t, A.pt, A.anodSteadyEnd);
        else
            x = A.t;
            xlab = 'Time (s)';
        ...
        if cbShowShading.Value
            vtResistanceWorkflow("shadeWindow", ax, cathStartX, cathEndX, [0.90 0.95 1.00], 0.12);
            vtResistanceWorkflow("shadeWindow", ax, anodStartX, anodEndX, [1.00 0.94 0.88], 0.12);
            vtResistanceWorkflow("shadeWindow", ax, cSteadyStartX, cSteadyEndX, [0.65 0.82 1.00], 0.22);
            vtResistanceWorkflow("shadeWindow", ax, aSteadyStartX, aSteadyEndX, [1.00 0.75 0.55], 0.22);
        end
        if cbShowMarkers.Value
            ...
            if startsWith(yChoice,'VT')
                vtResistanceWorkflow("addResistanceVTAnnotations", ax, A, cathBaseStartX, cathBaseEndX, anodBaseStartX, anodBaseEndX, ...
                    cSteadyStartX, cSteadyEndX, aSteadyStartX, aSteadyEndX, cathStartX, cathEndX, anodStartX, anodEndX);
            else
                vtResistanceWorkflow("addResistanceITAnnotations", ax, A, cSteadyStartX, cSteadyEndX, aSteadyStartX, aSteadyEndX, ...
