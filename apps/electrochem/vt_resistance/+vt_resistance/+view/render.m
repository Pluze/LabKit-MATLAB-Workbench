...
function renderBatchTable(state, ui)
    ...
    ui.controls.results.table.Data = ...
        vt_resistance.view.buildBatchTableData(state.items);
end
...
function renderResultsSummary(state, ui)
    ...
    ui.controls.cathBaseWindow.valueHandle.Value = ...
        vt_resistance.view.formatDurationUs(analysis.cathBaselineWindow_s);
    ui.controls.anodBaseWindow.valueHandle.Value = ...
        vt_resistance.view.formatDurationUs(analysis.anodBaselineWindow_s);
...
function plotOneAxis(ax, analysis, xChoice, yChoice, showGrid, itemName, ui)
    ...
    if ui.controls.showShading.valueHandle.Value
        vt_resistance.view.shadeWindow(ax, cathStartX, cathEndX, ...
            [0.90 0.95 1.00], 0.12);
        vt_resistance.view.shadeWindow(ax, anodStartX, anodEndX, ...
            [1.00 0.94 0.88], 0.12);
        vt_resistance.view.shadeWindow(ax, cSteadyStartX, cSteadyEndX, ...
            [0.65 0.82 1.00], 0.22);
        vt_resistance.view.shadeWindow(ax, aSteadyStartX, aSteadyEndX, ...
            [1.00 0.75 0.55], 0.22);
    end
    if ui.controls.showMarkers.valueHandle.Value
        ...
        if startsWith(yChoice, 'VT')
            vt_resistance.view.addResistanceVTAnnotations(ax, analysis, ...
            ...
            vt_resistance.view.addResistanceITAnnotations(ax, analysis, ...
                cSteadyStartX, cSteadyEndX, aSteadyStartX, aSteadyEndX, ...
                cathStartX, cathEndX, anodStartX, anodEndX);
        end
    end
