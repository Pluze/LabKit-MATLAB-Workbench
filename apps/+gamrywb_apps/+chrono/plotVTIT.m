...
%PLOTVTIT Plot voltage/current chrono overlays on paired axes.
    ...
    for k = 1:numel(items)
        ...
        plot(axV, x, voltage(item), 'LineWidth', opts.lineWidth, 'Color', cmap(k, :));
        plot(axI, x, current(item), 'LineWidth', opts.lineWidth, 'Color', cmap(k, :));
