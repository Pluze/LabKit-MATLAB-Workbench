...
%CHANNELROWS Build editable channel rows from RHS metadata.
    ...
    for k = 1:n
        ...
        [role(k), label(k), required(k)] = roleForChannel(roles, channel(k));
        if strlength(label(k)) == 0
            label(k) = channel(k);
        end
    end

    rows = table(preview, role, label, familyColumn, channel, required, unit, ...
...
function [roleId, roleLabel, required] = roleForChannel(roles, channelName)
    roleId = "";
    roleLabel = "";
    required = false;
    ...
    for k = 1:numel(roles)
        ...
        if any(normalizeName(aliases) == normalizedChannel)
            ...
            required = logical(fieldOrDefault(roles(k), "required", false));
            return;
        end
    end
end
