% Expected caller: rhs_preview.definitionActions and rhs_preview.userInterface.buildWorkbenchLayout. Input is
...
%SUMMARYTABLEDATA Build RHS preview summary rows.
    ...
    data = { ...
        ...
        'RHS folder', displayFile(S.rhsFolder); ...
        ...
        'Filter files', displayInteger(filterCount(S)); ...
        'Good files', displayInteger(goodFilterCount(S)); ...
...
function n = filterCount(S)
    n = NaN;
    if isfield(S, "filterRows") && istable(S.filterRows)
        n = height(S.filterRows);
    end
end

function n = goodFilterCount(S)
    n = NaN;
    if isfield(S, "filterRows") && istable(S.filterRows) && ...
            ismember("label", S.filterRows.Properties.VariableNames)
        n = sum(string(S.filterRows.label) == "good");
    end
end
