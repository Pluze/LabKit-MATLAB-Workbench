...
p.style = applyAspectPreset( ...
    p.style, p.aspectPreset, changedId, ...
...
defaults = figure_studio.styleLibrary.styleForPreset("Published figure");
...
if ~isfield(style, "outerMargin") || ...
        ~any(string(style.outerMargin) == marginChoices)
...
function style = applyAspectPreset(style, preset, changedId, sourceStyle)
...
if changedId == "aspectPreset" || changedId == "canvasWidth"
    ...
    style.canvasWidth = min(max(width, 320), 8000);
    style.canvasHeight = min(max(height, 240), 8000);
end
end
