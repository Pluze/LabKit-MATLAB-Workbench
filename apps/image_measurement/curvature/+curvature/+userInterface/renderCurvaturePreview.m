% Expected caller: the registered curvature V2 renderer. Inputs are one
% semantic image axes and a prepared model. Side effects are limited to
% non-pickable preview graphics on the supplied axes.
function renderCurvaturePreview(ax, model)
    ...
    ax.YDir = 'reverse';
    hold(ax, 'on');
    curvature.userInterface.plotStaticCurveAnchors( ...
...
function drawFit(ax, fit)
    ...
    plot(ax, fit.xc_px + fit.R_px .* cos(t), ...
        fit.yc_px + fit.R_px .* sin(t), 'r-', 'LineWidth', 2, ...
        'HitTest', 'off', 'PickableParts', 'none');
    plot(ax, fit.xc_px, fit.yc_px, 'ro', 'MarkerFaceColor', 'r', ...
        'HitTest', 'off', 'PickableParts', 'none');
end
...
function value = fitTitle(fit)
    ...
    if fit.ok
        value = sprintf('R = %.4g %s, curvature = %.4g %s, RMSE = %.3g %s', ...
