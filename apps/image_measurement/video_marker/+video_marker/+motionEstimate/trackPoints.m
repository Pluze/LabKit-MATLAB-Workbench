...
% return per-point confidence plus diagnostics. No third-party runtime is used.
function [points, confidence, diagnostics] = trackPoints( ...
    ...
    try
        % Resolve the guarded optional product only after availability is
        % confirmed so base-MATLAB dependency analysis does not load it.
        pointTrackerConstructor = str2func('vision.PointTracker');
        tracker = pointTrackerConstructor( ...
            'BlockSize', blockSize, ...
