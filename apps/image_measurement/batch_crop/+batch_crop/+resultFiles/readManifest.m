...
parameters = struct( ...
    ...
    "scaleMode", mode, ...
    "scaleUnit", unit, ...
    "physicalWidth", manifestPhysicalDimension( ...
        data, "PhysicalWidth", cropWidth(1), mode), ...
    "physicalHeight", manifestPhysicalDimension( ...
        data, "PhysicalHeight", cropHeight(1), mode), ...
...
if ~isfinite(parameters.physicalWidth) || ...
    ...
    error("batch_crop:InvalidManifest", ...
        "Manifest physical dimensions must be positive.");
end
if mode == "Physical"
...
function value = manifestPhysicalDimension(data, name, pixelFallback, mode)
if mode == "Physical"
    value = sharedNumeric(data, name);
    return
end
values = numericColumn(data, name);
if all(~isfinite(values))
    value = pixelFallback;
elseif all(isfinite(values)) && oneSharedValue(values)
    value = values(1);
else
    error("batch_crop:InvalidManifest", ...
        "Manifest shared settings must be consistent across rows.");
