...
function summary = scaleCalibrationSummary(items)
%SCALECALIBRATIONSUMMARY Summarize per-item scale calibration readiness.

    total = numel(items);
    calibrated = false(total, 1);
    for k = 1:total
        calibrated(k) = isfield(items(k), 'scaleCalibration') && ...
            batch_crop.appState.isScaleCalibrationSet(items(k).scaleCalibration);
    end
