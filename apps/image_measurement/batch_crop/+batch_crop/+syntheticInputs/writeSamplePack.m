...
%WRITESAMPLEPACK Write Batch Crop debug image files.
    ...
    project.inputs.sources = [ ...
        sampleContext.sourceRecord("image1", "cropSource", imageA), ...
        sampleContext.sourceRecord("image2", "cropSource", imageB)];
    project.inputs.items = batch_crop.cropTasks.forSourceIds( ...
        string({project.inputs.sources.id}));
    project.inputs.items(1).centerXY = [170 130];
...
function writeTextFile(filepath, text)
    ...
    if fid < 0
        error("batch_crop:syntheticInputs:SampleWriteFailed", ...
