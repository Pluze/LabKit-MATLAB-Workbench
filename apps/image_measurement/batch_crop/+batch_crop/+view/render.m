...
function renderList(state, ui)
    ...
    labkit.ui.view.setValue(ui, 'images', batch_crop.view.filePanelEntries( ...
    ...
    if strcmpi(string(ui.controls.scaleMode.valueHandle.Value), "Physical")
        ...
        ui.controls.imageStatus.valueHandle.Value = sprintf( ...
            ...
            numel(state.items), ...
            batch_crop.state.countConfirmedCenters(state.items), ...
...
function renderControls(state, ui, services)
    ...
    ui.controls.previousImage.button.Enable = batch_crop.view.ternary( ...
        hasImage && state.currentIndex > 1, 'on', 'off');
    ui.controls.nextImage.button.Enable = batch_crop.view.ternary( ...
    ...
    ui.controls.scaleUnit.valueHandle.Enable = ...
        batch_crop.view.ternary(physicalMode, 'on', 'off');
    ...
    if hasImage
        ...
        centerLimits = batch_crop.view.centerCoordinateLimits(geometry);
        ...
        labkit.ui.view.setValue(ui, "paddingPercent", ...
            batch_crop.state.itemPaddingPercent(item, ...
...
function renderPreview(state, ui)
    ...
    batch_crop.view.drawPreview(ui, state.tools.previewAxes, geometry, ...
...
function renderSummary(state, ui, services)
    ...
    ui.controls.resultTable.table.Data = batch_crop.view.summaryTableData( ...
        state, state.currentIndex, cropSize(1), cropSize(2), ...
        currentPaddingPercent(state, ui), ui.controls.format.valueHandle.Value);
    ui.controls.details.textArea.Value = batch_crop.view.detailLines( ...
        state, state.currentIndex, cropSize(1), cropSize(2), ...
...
function refreshScaleControls(state, ui, ~, statusControl)
    ...
    batch_crop.view.refreshScaleControls(state.tools.scaleTool, ...
