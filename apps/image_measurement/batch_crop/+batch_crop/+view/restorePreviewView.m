...
%RESTOREPREVIEWVIEW Restore source-coordinate preview limits after redraw.
    ...
    if hasOriginalCorners(state)
        ...
        for k = 1:size(state.originalCorners, 1)
            cornersCanvas(k, :) = batch_crop.ops.originalToCanvas(geometry, ...
...
function limits = originalAxisLimits(geometry, placement, originalLimits, axisIndex)
    ...
    firstCanvas = batch_crop.ops.originalToCanvas(geometry, firstPoint) + placement.offset;
    secondCanvas = batch_crop.ops.originalToCanvas(geometry, secondPoint) + placement.offset;
