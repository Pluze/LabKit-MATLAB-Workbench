...
%LABKIT_FOCUSSTACK_APP Fuse a focus image stack into one all-in-focus image.
    ...
    analysisPanel = labkit.ui.createPanelGrid(layFA, 'Fusion Options', 2, [5 2], ...
    ...
    chkRegister = uicheckbox(analysisGrid, ...
        ...
        'Value', false);
    chkRegister.Layout.Row = 1;
    chkRegister.Layout.Column = [1 2];

    [lblFocusWindow, edtFocusWindow] = labkit.ui.createLabeledSpinner(analysisGrid, ...
        'Focus window (px):', 'Value', 31, 'Limits', [3 99], 'Step', 2);
    lblFocusWindow.Layout.Row = 2;
    lblFocusWindow.Layout.Column = 1;
    edtFocusWindow.Layout.Row = 2;
    edtFocusWindow.Layout.Column = 2;

    [lblSmoothRadius, edtSmoothRadius] = labkit.ui.createLabeledSpinner(analysisGrid, ...
        'Mask smoothing (px):', 'Value', 4, 'Limits', [0 50], 'Step', 1);
    lblSmoothRadius.Layout.Row = 3;
    lblSmoothRadius.Layout.Column = 1;
    edtSmoothRadius.Layout.Row = 3;
    edtSmoothRadius.Layout.Column = 2;

    [lblMinConfidence, edtMinConfidence] = labkit.ui.createLabeledSpinner(analysisGrid, ...
        'Min confidence:', 'Value', 0.05, 'Limits', [0 1], 'Step', 0.05);
    lblMinConfidence.Layout.Row = 4;
    lblMinConfidence.Layout.Column = 1;
    edtMinConfidence.Layout.Row = 4;
    edtMinConfidence.Layout.Column = 2;
...
function T = buildFocusStackSummaryTable(result, paths)
    ...
    T = table( ...
        ...
        'FocusWindow_px', 'MaskSmoothing_px', 'MinConfidence'});
end
...
function data = focusStackResultTableData(result)
    ...
    data = { ...
        ...
        'Focus window', sprintf('%d px', result.focusWindow); ...
        'Mask smoothing', sprintf('%d px', result.smoothRadius); ...
...
function lines = focusStackDetails(result, paths, registrationLines)
    lines = { ...
        ...
        sprintf('Minimum confidence fallback: %.3g', result.minConfidence), ...
