% Expected caller: flir_thermal.definition during synthetic-input generation and unit tests. Input
...
%WRITESAMPLEPACK Write FLIR Thermal debug radiometric files.
    ...
    project.inputs.sources = [ ...
        sampleContext.sourceRecord( ...
            "thermal1", "thermal-image", warmPath), ...
        sampleContext.sourceRecord( ...
            "thermal2", "thermal-image", coolPath)];
    pack = labkit.app.synthetic.Pack( ...
        Scenario="representative-thermal-images", InitialInput=project, ...
        Artifacts={ ...
...
function writeSyntheticRjpeg(filepath, raw, opts)
    ...
    if fid < 0
        error("flir_thermal:syntheticInputs:SampleWriteFailed", ...
...
function bytes = jpegWrapper(block)
    ...
    if segmentLength > 65535
        error("flir_thermal:syntheticInputs:SampleTooLarge", ...
